...
| `docs/` (Sphinx source) | Given the same mechanical rename as the rest of the codebase. | Cosmetic only: this tree is not built or served by anything in this repo (see below), but it shouldn't say `xonsh` throughout a `pygwin` repo either. |
