...
| `ROADMAP.md` | Removed, after every item in it (phase one and phase two) was checked off. Its content is not lost: the outcome of each item lives on in `CHANGELOG.md`'s dated release entries, and the full investigation detail (methodology, numbers, what was deliberately left undone and why) stays in this repository's git history on the commits that closed each item out. | It was a live plan for work that is now finished; keeping a "roadmap" file around after its own last item is checked off just turns it into an inert history document duplicating what `CHANGELOG.md` already records, with none of the ongoing value a roadmap is supposed to have. |
| `xontrib/` -> `pgtrib/`, `xompletions/` -> `pgcompletions/`, `pygwin/xontribs.py` -> `pygwin/pgtribs.py`, `docs/xontrib.rst` -> `docs/pgtrib.rst`, plus every internal reference to either package (the `_load_xontrib_`/`_unload_xontrib_` plugin protocol, the `xontrib` alias command, the `pygwin.xontribs` entry-point group, the `$XONTRIBS_AUTOLOAD_DISABLED` env var, the `xonfig` wizard's xontrib section, the `xompleter` completion-trace tag, and the matching test files) renamed to `pgtrib`/`pgtribs`/`pgcompletions`/`pgcompleter`, at the project owner's explicit request, to finish moving off xonsh's own naming for pygwin's own mechanisms. | This is a deeper divergence than the original wholesale rename: xonsh's own plugin protocol name (`_load_xontrib_`) is now genuinely different from pygwin's (`_load_pgtrib_`), not just a renamed copy of the same string, so a real, unmodified xonsh xontrib (one xonsh's own community actually publishes) no longer has its `_load_xontrib_` entry point invoked at all when loaded in pygwin; pygwin's loader falls back to reading the module's `__all__`, which is a materially different, lesser result than actually running the plugin's real setup logic. This was already true in spirit (SYNCING.md has said third-party xontribs are not automatically compatible with pygwin since the original rename), but this rename makes that incompatibility a mechanical certainty for any xontrib that relies on `_load_xontrib_` doing real work, not just a stated policy. Real citations to xonsh's own ecosystem (`awesome-xontribs`, `xontrib-template`, the `coconut` xontrib, and similar) were deliberately left saying `xontrib`, not `pgtrib`, throughout `docs/`, `pygwin/webconfig/routes.py`, and code comments citing real upstream examples, for the same reason other real citations in this file are left alone: they name a real thing that is not pygwin's. |
