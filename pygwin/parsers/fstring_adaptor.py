"""Implements helper class for parsing Pygwin syntax within f-strings."""
...
def RE_FSTR_FIELD_WRAPPER():
    return re.compile(r"(__pygwin__\.eval_fstring_field\((\d+)\))\s*[^=]")
...
@lazyobject
def RE_PYGWIN_EXPR():
    """Matches pygwin-specific expressions: $NAME, $(...), ${...}, $[...],
    @(...), @$(...), @!(...), !(...), ![...]."""
...
def _extract_pygwin_expr(template, pos):
    """Extract a pygwin expression from template starting at or near pos."""
    m = RE_PYGWIN_EXPR.match(template, pos)
    if m is not None:
        return m.group()
    # The offset may point past a leading space: `{ $HOME }`
    if pos > 0:
        m = RE_PYGWIN_EXPR.match(template, pos - 1)
        if m is not None:
            return m.group()
    return None
...
class FStringAdaptor:
    """Helper for parsing Pygwin syntax within f-strings."""

    def __init__(self, fstring, prefix, filename=None):
        """Parses an f-string containing special Pygwin syntax and returns
    ...
    def _patch_special_syntax(self):
        ...
        pygwin expressions as its field values and substitues them for
        a call to __pygwin__.eval_fstring_field as needed.
        """
        ...
        while True:
            ...
            try:
                ...
                if PYTHON_VERSION_INFO < (3, 12):
                    ...
                    if epos < 0 or epos >= len(template):
                        raise
                    # Extract pygwin expression at this position
                    error_expr = _extract_pygwin_expr(template, epos)
            ...
            pygwin_field = (error_expr, self.filename if self.filename else None)
            self._field_counter += 1
            field_id = self._field_counter
            self.fields[field_id] = pygwin_field
            eval_field = f"__pygwin__.eval_fstring_field({field_id})"
