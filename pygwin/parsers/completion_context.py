"""Implements the pygwin (tab-)completion context parser.
...
from pygwin.parsers.base import Location, raise_parse_error
from pygwin.parsers.lexer import Lexer
from pygwin.parsers.ply import yacc
from pygwin.tools import check_for_partial_string, get_line_continuation
...
class CommandContext(NamedTuple):
    ...
    This affects the completion's behaviour - see ``Completer.complete`` in ``pygwin/completer.py``.
    """
...
class CompletionContextParser:
    ...
    def __init__(
        self,
        yacc_optimize=True,
        yacc_table="pygwin.completion_parser_table",
        debug=False,
        outputdir=None,
    ):
        self.cursor = 0
    ...
    def try_parse_string_literal(raw_arg: str) -> CommandArg | None:
        ...
        if startix != 0 or endix not in (
            ...
            # pygwin won't treat it as a string literal
            return None
        else:
            if endix is None:
                # no closing quote
