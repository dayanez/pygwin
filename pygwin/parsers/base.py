"""Implements the base pygwin parser."""
...
from pygwin.parsers.ast import has_elts, load_attribute_chain, pygwin_call
from pygwin.parsers.context_check import check_contexts
from pygwin.parsers.fstring_adaptor import FStringAdaptor
from pygwin.parsers.lexer import Lexer, LexToken
from pygwin.parsers.ply import yacc
from pygwin.parsers.tokenize import SearchPath, StringPrefix
...
# subject to ``$PYGWIN_SUBPROC_RAISE_ERROR``.  Per spec only the explicit
...
def _is_subproc_helper_call(node):
    """True if `node` is an `ast.Call` to a `__pygwin__.subproc_*` helper."""
...
def _is_raising_subproc_helper_call(node):
    ...
    that should raise on non-zero rc when ``$PYGWIN_SUBPROC_RAISE_ERROR``
    is True.
    """
...
def _is_subproc_check_boolop_call(node):
    ...
    return (
        ...
        and node.func.value.id == "__pygwin__"
    )
...
class _SubprocChainRaiseWrapper:
    ...
    ``__pygwin__.subproc_check_boolop(...)`` so the runtime can enforce
    ...
    def _wrap(node):
        ...
        return ast.Call(
            func=load_attribute_chain(
                "__pygwin__.subproc_check_boolop", lineno=lineno, col=col
            ),
            args=[node],
            keywords=[],
            lineno=lineno,
...
def wrap_subproc_raise_checks(tree):
    ...
    *and* standalone "uncaptured" subproc-helper Calls (bare commands,
    ``![...]``, ``$[...]``) in ``__pygwin__.subproc_check_boolop`` so the
    result is checked at runtime against ``$PYGWIN_SUBPROC_RAISE_ERROR``.
    """
...
def pygwin_help(x, lineno=None, col=None):
    """Creates the AST node for calling the __pygwin__.help() function."""
    return pygwin_call("__pygwin__.help", [x], lineno=lineno, col=col)


def pygwin_superhelp(x, lineno=None, col=None):
    """Creates the AST node for calling the __pygwin__.superhelp() function."""
    return pygwin_call("__pygwin__.superhelp", [x], lineno=lineno, col=col)
...
class BaseParser:
    """A base class that parses the pygwin language."""

    def __init__(
        self,
        yacc_optimize=True,
        yacc_table="pygwin.parser_table",
        yacc_debug=False,
        outputdir=None,
    ):
        """Parameters
    ...
    def parse(self, s, filename="<code>", mode="exec", debug_level=0):
        """Returns an abstract syntax tree of pygwin code.
    ...
    def pygwin_pathsearch(self, pattern, pymode=False, lineno=None, col=None):
        """Creates the AST node for calling the __pygwin__.pathsearch() function.
    ...
    def p_atom_atdot_name(self, p):
        ...
        pygwin_obj = load_attribute_chain(
            "__pygwin__.interface", lineno=lineno, col=col
        )
        p[0] = ast.Attribute(
            value=pygwin_obj,
            attr=p3.value,
    ...
    def p_atom_at(self, p):
        ...
        p1 = p[1]
        lineno, col = p1.lineno, p1.lexpos
        p[0] = load_attribute_chain("__pygwin__.interface", lineno=lineno, col=col)

    def _pygwin_attr_chain(self, lineno, col, first_attr, more_attrs=None):
        """
        Building ast.Attribute-chain: __pygwin__.first_attr[.more_attrs...]
        """
        node = load_attribute_chain("__pygwin__.interface", lineno=lineno, col=col)
    ...
    def p_decorator_atat_nocall_simple(self, p):
        ...
        target = self._pygwin_attr_chain(lineno, col, p[4], [])
        p[0] = target
    ...
    def p_decorator_atat_nocall_chain(self, p):
        ...
        at2 = p[2]
        lineno, col = at2.lineno, at2.lexpos
        target = self._pygwin_attr_chain(lineno, col, p[4], p[5])
        p[0] = target
    ...
    def p_with_bang_stmt_single_suite(self, p):
        ...
        gblcall = pygwin_call("globals", [], lineno=l, col=c)
        loccall = pygwin_call("locals", [], lineno=l, col=c)
        margs = [expr, p4, gblcall, loccall]
        p3.context_expr = pygwin_call("__pygwin__.enter_macro", margs, lineno=l, col=c)
    ...
    def p_with_bang_stmt_many_suite(self, p):
        ...
        for item in items:
            ...
            item.context_expr = pygwin_call(
                "__pygwin__.enter_macro", margs, lineno=l, col=c
            )
    ...
    def apply_trailers(self, leader, trailers):
        ...
        for trailer in trailers:
            if isinstance(
                ...
                p0 = pygwin_call("__pygwin__.call_macro", margs, lineno=l, col=c)
    ...
    def p_string_literal(self, p):
        ...
        if "p" in prefix and "f" in prefix:
            ...
            p[0] = pygwin_call(
                "__pygwin__.path_literal", [s], lineno=p1.lineno, col=p1.lexpos
            )
        elif "p" in prefix:
    ...
    def p_string_literal_list(self, p):
        ...
        if len(p) == 3:
            ...
            if p1_type == p2_type:
                if p1_type == "path":
                    # the nodes are function calls to __pygwin__.path_literal()
    ...
    def _dollar_rules(self, p):
        """These handle the special pygwin $ shell atoms by looking up
        in a special __pygwin__.env dictionary injected in the __builtin__.
        """
        lenp = len(p)
        p1, p2 = p[1], p[2]
        ...
        if lenp == 3:  # $NAME
            ...
            xenv = load_attribute_chain("__pygwin__.env", lineno=lineno, col=col)
            ...
            p0 = pygwin_call(
                "__pygwin__.subproc_captured_stdout", p2, lineno=lineno, col=col
            )
        elif p1 == "!(":
            p0 = pygwin_call(
                "__pygwin__.subproc_captured_object", p2, lineno=lineno, col=col
            )
        elif p1 == "![":
            p0 = pygwin_call(
                "__pygwin__.subproc_captured_hiddenobject", p2, lineno=lineno, col=col
            )
        elif p1 == "$[":
            p0 = pygwin_call(
                "__pygwin__.subproc_uncaptured", p2, lineno=lineno, col=col
            )
        else:
            raise AssertionError()
    ...
    def _envvar_by_name(self, var, lineno=None, col=None):
        """Looks up a pygwin variable by name."""
        xenv = load_attribute_chain("__pygwin__.env", lineno=lineno, col=col)
    ...
    def _envvar_set_call(self, node, value):
        """Creates __pygwin__.env.set(key, value) AST from an envvar Subscript node."""
        key = self._envvar_node_key(node)
        return ast.Call(
            func=load_attribute_chain(
                "__pygwin__.env.set", lineno=node.lineno, col=node.col_offset
            ),
            args=[key, value],
            keywords=[],
            lineno=node.lineno,
    ...
    def _is_envvar_node(node):
        """Check if an AST node is __pygwin__.env[KEY]."""
        return (
            isinstance(node, ast.Subscript)
            and isinstance(node.value, ast.Attribute)
            and node.value.attr == "env"
            and isinstance(node.value.value, ast.Name)
            and node.value.value.id == "__pygwin__"
        )

    @staticmethod
    def _envvar_node_key(node):
        """Extract the key AST node from __pygwin__.env[KEY]."""
        s = node.slice
    ...
    def p_subproc_atoms_subshell(self, p):
        ...
        # *this* pygwin, expressed as ``[sys.executable, "-m", "pygwin"]``.
        # The literal ``"pygwin"`` we used to emit here required the
        # ``pygwin`` console-script to be on PATH; that breaks every
        # ``(...)`` subshell when pygwin is reachable only as
        # ``python -m pygwin`` (FreeBSD jails, ad-hoc venvs, ``pipx run
        # pygwin``, …). Resolve at runtime via ``__import__('sys')``
        ...
        self_argv = ast.List(
            elts=[
                ...
                ast.const_str(s="pygwin", lineno=l, col_offset=c),
            ],
            ctx=ast.Load(),
            lineno=l,
            col_offset=c,
    ...
    def p_subproc_atom_uncaptured(self, p):
        ...
        p1 = p[1]
        p0 = pygwin_call(
            "__pygwin__.subproc_uncaptured", args=p[2], lineno=p1.lineno, col=p1.lexpos
        )
    ...
    def p_subproc_atom_pyeval(self, p):
        ...
        p1 = p[1]
        p0 = pygwin_call(
            "__pygwin__.list_of_strs_or_callables",
            [p[2]],
            lineno=p1.lineno,
            col=p1.lexpos,
        )
    ...
    def p_subproc_atom_subproc_inject(self, p):
        ...
        p1, p2 = p[1], p[2]
        p0 = pygwin_call(
            "__pygwin__.subproc_captured_inject", p2, lineno=p1.lineno, col=p1.lexpos
        )
        p0._cliarg_action = "extend"
    ...
    def p_subproc_atom_re(self, p):
        ...
        p0 = self.pygwin_pathsearch(
            p[1], pymode=False, lineno=self.lineno, col=self.col
        )
        p0._cliarg_action = "extend"
        p[0] = p0
    ...
    def p_subproc_atom_str(self, p):
        ...
        if hasattr(p[1], "is_raw") and p[1].is_raw:
            ...
            p0 = pygwin_call(
                "__pygwin__.expand_path", args=[p[1]], lineno=self.lineno, col=self.col
            )
        p0._cliarg_action = "append"
        p[0] = p0
    ...
    def p_subproc_atom_arg(self, p):
        ...
        if isinstance(p1, list):
            ...
            p0 = pygwin_call(
                "__pygwin__.list_of_list_of_strs_outer_product",
            ...
            p0 = pygwin_call(
                "__pygwin__.glob", args=[p1], lineno=p1.lineno, col=p1.col_offset
            )
            p0._cliarg_action = "extend"
        else:
            # literal str argument
            p0 = pygwin_call(
                "__pygwin__.expand_path", args=[p1], lineno=p1.lineno, col=p1.col_offset
            )
            p0._cliarg_action = "append"
        p[0] = p0
