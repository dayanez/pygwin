...
from pygwin.parsers.completion_context import CompletionContext, CompletionContextParser
from pygwin.tools import print_above_prompt, print_exception
...
class Completer:
    """This provides a list of optional completions for the pygwin shell."""
    ...
    def _format_trace_item(comp, lprefix, source: str, exclusive: bool) -> str:
        """Render one completion as a single ``$PYGWIN_COMPLETER_TRACE`` line.
    ...
    def complete_from_context(self, completion_context, old_completer_args=None):
        trace = XSH.env.get("PYGWIN_COMPLETER_TRACE")
        if trace:
