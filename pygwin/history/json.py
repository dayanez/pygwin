"""Implements JSON version of pygwin history backend."""
...
import pygwin.lib.lazyjson as xlj
import pygwin.tools as xt
import pygwin.xoreutils.uptime as uptime
...
def _xhj_get_data_dir():
    dir = xt.expanduser_abs_path(
        os.path.join(XSH.env.get("PYGWIN_DATA_DIR"), "history_json")
    )
...
def _xhj_get_data_dir_files(data_dir, include_mtime=False):
    ...
    try:
        ...
        if XSH.env.get("PYGWIN_DEBUG"):
            xt.print_exception(
                f"Could not collect pygwin history json files from {data_dir}"
            )
        return
    for file in entries:
        if not (file.startswith("pygwin-") and file.endswith(".json")):
            continue
        ...
        if include_mtime:
            try:
                ...
                if XSH.env.get("PYGWIN_DEBUG"):
                    print(
                        f"pygwin history: skipping unreadable file {fullpath!r}: {err}",
                        file=sys.stderr,
                    )
                continue
        else:
...
def _xhj_get_history_files(sort=True, newest_first=False, modified_since=None):
    ...
    data_dirs = [
        _xhj_get_data_dir(),
        XSH.env.get("PYGWIN_DATA_DIR"),  # backwards compatibility, remove in the future
    ]
    ...
    custom_history_file = XSH.env.get("PYGWIN_HISTORY_FILE", None)
...
def _xhj_pull_items(pull_times, src_sessionid=None):
    ...
    if src_sessionid:
        filename = os.path.join(_xhj_get_data_dir(), f"pygwin-{src_sessionid}.json")
        src_paths = [filename]
    else:
...
class JsonHistoryGC(threading.Thread):
    ...
    def __init__(self, wait_for_shell=True, size=None, force=False, *args, **kwargs):
        ...
        May wait for shell (and for pygwinrc to have been loaded) to start work.
        """
    ...
    def run(self):
        ...
        pygwin_debug = env.get("PYGWIN_DEBUG", 0)
        if self.size is None:
            hsize, units = env.get("PYGWIN_HISTORY_SIZE")
        else:
        ...
        if self.force_gc or size_over < hsize:
            ...
            print(
                ...
                "Not removing any history for now. Either increase your limit ($PYGWIN_HISTORY_SIZE), or run `history gc --force`."
            )
...
class JsonHistoryFlusher(threading.Thread):
    ...
    def dump(self):
        ...
        if self.at_exit:
            ...
            hist["locked"] = False
        if not XSH.env.get("PYGWIN_STORE_STDOUT", False):
...
class JsonHistory(History):
    """Pygwin history backend implemented with JSON files.
    ...
    def __init__(
        ...
        **meta,
        ...
        """Represents a pygwin session's history as an in-memory buffer that is
        ...
        filename : str, optional
            ...
            ``$PYGWIN_DATA_DIR/history_json/pygwin-{sessionid}.json``.
        ...
        if filename is None:
            ...
            self.filename = os.path.join(data_dir, f"pygwin-{self.sessionid}.json")
        else:
            self.filename = filename
        ...
        self.save_cwd = (
            ...
            else XSH.env.get("PYGWIN_HISTORY_SAVE_CWD", True)
        )
    ...
    def all_items(self, newest_first=False, **kwargs):
        """
        Returns all history as found in PYGWIN_DATA_DIR.
        ...
        for f in _xhj_get_history_files(newest_first=newest_first):
            ...
            try:
                ...
                if XSH.env.get("PYGWIN_DEBUG", 0) > 0:
                    msg = "pygwin history file {0!r} is not valid JSON"
    ...
    def info(self):
        ...
        envs = XSH.env
        data["gc options"] = envs.get("PYGWIN_HISTORY_SIZE")
