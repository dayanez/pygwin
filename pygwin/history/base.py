"""Base class of Pygwin History backends."""
...
class HistoryEntry(types.SimpleNamespace):
    ...
    out: str
        The output of the command, if pygwin is configured to save it
    rtn: int
...
class History:
    """Pygwin history backend base class.
    ...
    def __init__(self, sessionid=None, **kwargs):
        """Represents a pygwin session's history.
    ...
    def ignore_regex(self):
        compiled_regex = None
        regex = XSH.env.get("PYGWIN_HISTORY_IGNORE_REGEX")
        if regex:
            try:
                compiled_regex = re.compile(regex)
            except re.error:
                print_warning(
                    "PYGWIN_HISTORY_IGNORE_REGEX is not a valid regular expression and will be ignored"
                )
