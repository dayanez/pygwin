"""Implements the pygwin history backend via sqlite3."""
...
XH_SQLITE_TABLE_NAME = "pygwin_history"
XH_SQLITE_CREATED_SQL_TBL = "CREATED_SQL_TABLE"


def _xh_sqlite_get_file_name():
    envs = XSH.env
    file_name = envs.get("PYGWIN_HISTORY_SQLITE_FILE")
    if not file_name:
        data_dir = envs.get("PYGWIN_DATA_DIR")
        file_name = os.path.join(data_dir, "pygwin-history.sqlite")
...
class SqliteHistoryGC(threading.Thread):
    ...
    def run(self):
        ...
        if self.size is not None:
            ...
            envs = XSH.env
            hsize, units = envs.get("PYGWIN_HISTORY_SIZE")
        if units != "commands":
            import warnings

            warnings.warn(
                f"$PYGWIN_HISTORY_SIZE unit {units!r} is not supported "
    ...
    def append(self, cmd):
        ...
        try:
            xh_sqlite_append_history(
                cmd,
                str(self.sessionid),
                store_stdout=envs.get("PYGWIN_STORE_STDOUT", False),
