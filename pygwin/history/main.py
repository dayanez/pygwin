"""Main entry points of the pygwin history."""
...
import pygwin.history.diff_history as xdh
...
def _xh_get_history(
    ...
    *,
    ...
    Parameters
    ----------
    session: {'session', 'all', 'pygwin', 'bash', 'zsh'}
...
class HistoryAlias(xcli.ArgParserAlias):
    ...
    def show(
        ...
        ----------
        session:
            The history session to get. (all is an alias for pygwin)
        slices:
        ...
        start_time: --start-time, +T
            ...
            Accepts $PYGWIN_DATETIME_FORMAT or ISO-8601 (e.g. 2023-07-17)
        end_time: -T, --end-time
    ...
    def pull(show_commands=False, session_id=None, _stdout=None):
        ...
        if hist.pull.__module__ == "pygwin.history.base":
            backend = XSH.env.get("PYGWIN_HISTORY_BACKEND", "unknown")
            print(
    ...
    def transfer(
        ...
        target : --target, -t
            Name of the target history backend. (default: $PYGWIN_HISTORY_BACKEND)
    ...
    def __call__(self, args, *rest, **kwargs):
        if XSH.history is None:
            ...
            # ``pygwin.main.start_services``). If a history command runs before
