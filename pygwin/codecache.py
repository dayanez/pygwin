...
from pygwin.tools import is_writable_file, print_warning
...
def should_use_cache(execer, mode):
    ...
    if mode == "exec":
        return (execer.scriptcache or execer.cacheall) and (
            XSH.env["PYGWIN_CACHE_SCRIPTS"] or XSH.env["PYGWIN_CACHE_EVERYTHING"]
        )
    else:
        return execer.cacheall or XSH.env["PYGWIN_CACHE_EVERYTHING"]
...
def get_cache_filename(fname, code=True):
    ...
    """
    datadir = XSH.env["PYGWIN_DATA_DIR"]
    cachedir = os.path.join(
        datadir, "pygwin_code_cache" if code else "pygwin_script_cache"
    )
        ...
        if not writable:
            if XSH.env.get("PYGWIN_DEBUG", "False"):
                print_warning(
                    ...
                    f"Set $PYGWIN_CACHE_SCRIPTS=0, $PYGWIN_CACHE_EVERYTHING=0 to disable cache."
                )
            return
        with open(cache_file_name, "wb") as cfile:
            cfile.write(PYGWIN_VERSION.encode() + b"\n")
