...
from pygwin.completers.completer import (
    add_one_completer,
    list_completers,
    remove_completer,
)
...
def _register_completer(
    ...
    func
        the name of a completer function to use.  This should be a function
         ...
         ``pygwin.completers.tools.contextual_completer`` decorator.
...
class CompleterAlias(xcli.ArgParserAlias):
    """CLI to add/remove/list pygwin auto-complete functions"""
...
def complete_aliases(command: CommandContext):
    """Complete any alias that has ``pygwin_complete`` attribute.
    ...
    completer = getattr(alias, "pygwin_complete", None)
    if not completer:
        from pygwin.aliases import FuncAlias

        if isinstance(alias, FuncAlias):
            completer = getattr(alias.func, "pygwin_complete", None)
    if not completer:
        return
