...
@xl.lazyobject
def PYGWIN_TOKENS():
    return set(PYGWIN_EXPR_TOKENS) | set(PYGWIN_STMT_TOKENS)


@xl.lazyobject
def RE_PYGWIN_IMP():
    """Regex pattern to match __pygwin__.imp.<module> syntax."""
    return re.compile(r"(__pygwin__|@)\.imp\.([a-zA-Z_][\w\.]*)?$")


@contextual_completer
def complete_pygwin_imp(context: CompletionContext) -> CompleterResult:
    """
    Completes module names for the inline pygwin importer (@.imp.<module>).
    """
    ...
    # Check if we're completing __pygwin__.imp.<module>
    m = RE_PYGWIN_IMP.search(prefix)
    if m is None:
        return None
    ...
    from pygwin.completers.imports import (
        filter_completions,
        get_root_modules,
        try_import,
    )
...
def complete_python(context: CompletionContext) -> CompleterResult:
    ...
    the Python built-ins, and pygwin operators.
    """
...
def _complete_python(prefix, context: PythonContext):
    ...
    if len(args) == 0:
        ...
        rtn |= {s for s in PYGWIN_TOKENS if filt(s, prefix)}
    else:
        rtn |= {s for s in PYGWIN_EXPR_TOKENS if filt(s, prefix)}
    ...
    if not prefix:
        ...
        # builtin (dunders included) and every ``PYGWIN_TOKENS`` entry
...
def _safe_eval(expr, ctx):
    ...
    """
    _ctx = None
    pygwin_safe_eval = XSH.execer.eval
    try:
        val = pygwin_safe_eval(expr, ctx, ctx, transform=False)
        _ctx = ctx
    except Exception:
        try:
            val = pygwin_safe_eval(expr, builtins.__dict__, transform=False)
...
def attr_complete(prefix, ctx, filter_func):
    ...
    expr = xt.subexpr_from_unbalanced(expr, "{", "}")
    if expr.startswith("@."):
        # @.sub -> __pygwin__.interface.sub
        expr = "__pygwin__.interface" + expr[1:]
    ...
    from pygwin.procs.pipelines import CommandPipeline, blocking_property
