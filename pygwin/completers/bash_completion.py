...
def _bash_completion_paths_default():
    ...
    Delegates to :data:`pygwin.platform_info.BASH_COMPLETIONS_DEFAULT` — the
    ...
    """
    from pygwin.platform_info import BASH_COMPLETIONS_DEFAULT
    ...
    for path in map(pathlib.Path, paths):
        ...
        if path.is_dir():
            try:
                ...
                if XSH.env and XSH.env.get("PYGWIN_COMPLETER_TRACE"):
...
def _bash_unescape(s):
    ...
    quoting context. pygwin's own quoting (single quotes, raw strings) makes
