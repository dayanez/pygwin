...
- ``$PYGWIN_COMPLETER_EMOJI_PREFIX`` (default ``"::"``) — colorful emoji (faces, animals, objects).
- ``$PYGWIN_COMPLETER_SYMBOLS_PREFIX`` (default ``":::"``) — simple unicode symbols (arrows, math, dingbats).
...
from pygwin.completers.tools import RichCompletion, contextual_command_completer
...
def get_symbol_cache():
    ...
    Examples
    --------
    >>> from pygwin.completers.emoji import get_symbol_cache
...
def _search(cache, query, prefix_len=0):
    ...
    Examples
    --------
    >>> from pygwin.completers.emoji import get_symbol_cache, _search
...
def complete_emoji(ctx: CommandContext):
    ...
    symbol_trigger = env.get("PYGWIN_COMPLETER_SYMBOLS_PREFIX") or ""
    emoji_trigger = env.get("PYGWIN_COMPLETER_EMOJI_PREFIX") or ""
