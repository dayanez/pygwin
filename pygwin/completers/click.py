...
:func:`pygwin.aliases.Aliases.register_click_command`.
...
alias wrapper as ``pygwin_complete``. The stock alias completer
(``pygwin.completers._aliases.complete_aliases``) then picks it up from
``alias.func.pygwin_complete`` during normal tab completion.
...
def complete_click(click_cmd, command, alias=None, **_):
    ...
    :func:`pygwin.aliases._click_command_alias`, so each wrapper captures
