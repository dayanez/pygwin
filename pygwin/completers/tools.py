...
def get_filter_function():
    """Return the completion filter function based on ``$PYGWIN_COMPLETER_MODE``.
    ...
    * ``"prefix"`` — only completions that start with the prefix are
    ...
    mode = (XSH.env or {}).get("PYGWIN_COMPLETER_MODE", "substring_tier")
    if mode == "prefix":
...
class RichCompletion(str):
    ...
    def __init__(
        ...
        provider :
            ...
            ``$PYGWIN_COMPLETER_TRACE`` so users can tell whether a match
...
def contextual_completer(func: ContextualCompleter):
    ...
    See ``pygwin/parsers/completion_context.py``.
...
def tag_provider(result, provider: str):
    """Tag completer output with ``provider`` for ``$PYGWIN_COMPLETER_TRACE``.
