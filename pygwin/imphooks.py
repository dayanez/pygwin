"""Import hooks for importing pygwin source files.
...
from pygwin.tools import print_warning
...
class PygwinImportHook(MetaPathFinder, SourceLoader):  # type: ignore
    """Implements the import hook for pygwin source files."""
    ...
    def find_spec(self, fullname, path, target=None):
        """Finds the spec for a pygwin module if it exists."""
        dot = "."
        spec = None
    ...
    def create_module(self, spec):
        """Create a pygwin module with the appropriate attributes."""
    ...
    def get_code(self, fullname):
        """Gets the code object for a pygwin file."""
        filename = self.get_filename(fullname)
        if filename is None:
            raise ImportError(f"pygwin file {fullname!r} could not be found")
        execer = self._execer
...
def _should_dispatch_pygwin_import_event_loader():
...
class PygwinImportEventHook(MetaPathFinder):
    """Implements the import hook for firing pygwin events on import."""
    ...
    def find_spec(self, fullname, path, target=None):
        ...
        dispatch_load = _should_dispatch_pygwin_import_event_loader()
        if npre > 0:
        ...
        if dispatch_load and spec is not None and hasattr(spec.loader, "create_module"):
            spec.loader = PygwinImportEventLoader(spec.loader)
        return spec
...
class PygwinImportEventLoader(Loader):
...
def install_import_hooks(execer=ARG_NOT_PRESENT):
    """
    Install Pygwin import hooks in ``sys.meta_path`` in order for ``.xsh`` files
    to be importable and import events to be fired.

    Can safely be called many times, will be no-op if pygwin import hooks are
    already present.
    """
    ...
    for hook in sys.meta_path:
        if isinstance(hook, PygwinImportHook):
            found_imp = True
        elif isinstance(hook, PygwinImportEventHook):
            found_event = True
    if not found_imp:
        sys.meta_path.append(PygwinImportHook(execer))
    if not found_event:
        sys.meta_path.insert(0, PygwinImportEventHook())
