...
from pygwin.tools import PygwinError, is_class, print_exception, simple_random_choice
...
.. code-block:: pygwin
    ...
    def _pipe_prev_command(cmd, **kw):
        ...
        if cmd and cmd.startswith('| ') and __pygwin__.history:
            return __pygwin__.history[-1].cmd.rstrip() + cmd.rstrip()
        return cmd
...
.. code-block:: pygwin
    ...
    def _audit(cmd, **kw):
        ...
        with open('/tmp/pygwin_audit.log', 'a') as f:
            f.write(cmd)
""",
)
...
* ``out``: If pygwin stores command output, this is the output
...
.. code-block:: pygwin
    ...
    def _prompt_err_command_again(cmd, rtn, out, ts):
        ...
        if rtn != 0:
            $PYGWIN_PROMPT_NEXT_CMD = cmd.rstrip()
""",
)
...
def transform_command(src, show_diff=True):
    ...
    debug_level = XSH.env.get("PYGWIN_DEBUG")
...
class Shell:
    ...
    def choose_shell_type(init_shell_type=None, env=None):
        ...
        if shell_type is None and env:
            ...
            if shell_type == "none":
                # This bricks interactive pygwin
    ...
    def construct_shell_cls(backend, **kwargs):
        ...
        if is_class(backend):
            ...
            `echo 'echo 1' | pygwin -i` and it's not working with `TERM=dumb` (#5462 #5517)
            ...
            is_stdin_to_interactive = (
                XSH.env.get("PYGWIN_INTERACTIVE", False) and not sys.stdin.isatty()
            )

            if backend == "none":
                from pygwin.shells.base_shell import BaseShell as cls
            elif backend == "prompt_toolkit" or is_stdin_to_interactive:
                from pygwin.shells.ptk_shell import PromptToolkitShell as cls
            elif backend == "readline":
                from pygwin.shells.readline_shell import ReadlineShell as cls
            elif backend == "dumb":
                from pygwin.shells.dumb_shell import DumbShell as cls
            else:
                raise PygwinError(f"{backend} is not recognized as a shell type")
        return cls(**kwargs)
    ...
    def __init__(
        ...
        ----------
        execer : Execer
            An execer instance capable of running pygwin code.
        ...
        if env.get("PYGWIN_INTERACTIVE"):
            XSH.history = hist = xhm.construct_history(
                env=env.detype(),
                ts=[time.time(), None],
                locked=True,
                filename=env.get("PYGWIN_HISTORY_FILE", None),
                sessionid=XSH.sessionid,
                backend=history_backend,
            )
            env["PYGWIN_HISTORY_FILE"] = hist.filename
        else:
            XSH.history = hist = DummyHistory()
            env["PYGWIN_HISTORY_FILE"] = None
        ...
        if env.get("PYGWIN_INTERACTIVE"):
            from pygwin.prompt.env import emit_osc7
