...
import pygwin.completers._aliases as xca
import pygwin.history.main as xhm
import pygwin.xoreutils.which as xxw
import pygwin.xoreutils.xcontext as xxt
from pygwin.built_ins import XSH
from pygwin.cli_utils import Annotated, Arg, ArgParserAlias
from pygwin.dirstack import _get_cwd, cd, dirs, popd, pushd
from pygwin.environ import locate_binary, make_args_env
...
from pygwin.procs.executables import locate_file
from pygwin.procs.jobs import bg, clean_jobs, disown, fg, jobs
from pygwin.procs.specs import DecoratorAlias, SpecAttrDecoratorAlias
...
def print_alias_help(name: str, superhelp: bool = False) -> None:
    ...
    Output is colorized via pygwin color tokens.
    ...
    """
    import pygwin.tools as xt
    from pygwin.procs.executables import locate_executable
    ...
    if doc:
        from pygwin.environ import _rst_inline_to_color
...
class Aliases(cabc.MutableMapping):
    ...
    def __getattr__(self, name):
        ...
        # pygwin sessions that never use click don't pay the import cost.
    ...
    def _add_click_command(self):
        ...
        * ``self.click`` — the ``click`` module itself, for convenient
          access from pygwinrc / user scripts.
    ...
    def completer(completer_func):
        ...
        def decorator(func):
            func.pygwin_complete = completer_func
            return func
    ...
    def get_doc(self, name: str) -> str:
        ...
        2. For ``FuncAlias`` instances, the wrapped function's
           ...
           class placeholder ``"Provides a callable alias for pygwin
           commands."``).
        ...
        if func is not None:
            ...
            # for pygwin source code.") that would otherwise leak through as
...
class ExecAlias:
    """Provides an exec alias for pygwin source code."""
...
def _click_command_alias(func_or_name=None, *, _aliases):
    """Decorator factory that registers a pygwin alias as a ``click`` command.
    ...
    * ``@aliases.register_click_command("custom-name")`` — explicit alias name.
    ...
    carrying the usual pygwin alias params (``alias_args``, ``stdin``, ``stdout``,
    ...
    def _make_alias(func, alias_name=None):
        ...
        class PygwinContext(click.Context):
        ...
        # function's name.)
        PygwinContext.click = click

        cmd.context_class = PygwinContext
        ...
        # ``alias.func.pygwin_complete``. Bound to the live click.Command so
        # the completer always reflects the current option/argument set —
        # even if the user mutates ``cmd.params`` after registration.
        from pygwin.completers.click import complete_click

        _wrapper.pygwin_complete = functools.partial(complete_click, cmd)
...
def pygwin_exit(args, stdin=None):
...
def pygwin_reset(args, stdin=None):
    """Clears __pygwin__.ctx"""
    XSH.ctx.clear()
...
def source_foreign_fn(
    ...
    overwrite_aliases : --overwrite-aliases
        flag for whether or not sourced aliases should replace the current pygwin aliases.
    ...
    show_output : --show-output
        ...
        pygwin terminal. By default they are silently discarded.
    dryrun : -d, --dry-run
    ...
    if prevcmd:
        ...
        if not sourcer:
            ...
            # file argument was given, even though every shell pygwin
        ...
        if not sourcer:
            return (
                None,
                f"pygwin: error: cannot determine how to source files for "
    ...
    if fsenv is None:
        if dryrun:
            ...
            msg = (
                f"pygwin: error: Failed to source: {what}\n"
                "pygwin: error: The foreign shell exited with an error "
    ...
    for k, v in fsaliases.items():
        if k in baliases and v == baliases[k]:
            ...
            msg = (
                ...
                "since it shares a name with an existing pygwin alias. "
...
def source_alias_fn(
    ...
    """
    env = XSH.env
    encoding = env.get("PYGWIN_ENCODING")
    errors = env.get("PYGWIN_ENCODING_ERRORS")
    for i, fname in enumerate(files):
        fpath = fname
        if not os.path.isfile(fpath):
            fpath = locate_file(fname)
            if fpath is None:
                if env.get("PYGWIN_DEBUG"):
        ...
        if not ignore_ext and fext not in {".xsh", ".py", ".pygwinrc"}:
            raise RuntimeError(
                f"Attempting to source file with non-pygwin extension {name!r}! "
        ...
        with (
            env.swap(PYGWIN_MODE="source", **make_args_env(files[i + 1 :])),
            ...
            try:
                ...
                print_color(
                    "You may be attempting to source non-pygwin file: "
                    f"{fpath!r}. "
...
def source_cmd_fn(
    ...
    if not os.path.isfile(args[0]):
        return (None, f"pygwin: error: File not found: {args[0]}\n", 1)
    prevcmd = "call "
...
def xexec_fn(
    ...
    replace the pygwin process with the specified program.
    ...
    """
    if len(command) == 0:
        return (None, "pygwin: exec: no command specified\n", 1)
    ...
    # (https://github.com/pygwin/pygwin/pull/6198)
    ...
    try:
        ...
        if e.errno == 8:  # Exec format error — not a binary, try shebang
            from pygwin.procs.specs import get_script_subproc_command
        ...
        if e.errno == 2:  # FileNotFoundError
            return (
                None,
                f"pygwin: exec: file not found: {e.args[1]}: {command[0]}\n",
                1,
            )
        return (
            None,
            f"pygwin: exec: {e.args[1]}: {command[0]}\n",
            1,
        )
...
@lazyobject
def xonfig():
    """Runs the pygwin configuration utility."""
    from pygwin.xonfig import xonfig_main  # lazy import
...
def trace(args, stdin=None, stdout=None, stderr=None, spec=None):
    """Runs the pygwin tracer utility."""
    from pygwin.tracer import tracermain  # lazy import
...
def showcmd(args, stdin=None):
    ...
    Displays the command and arguments as a list of strings that pygwin would
    run in subprocess mode. This is useful for determining how pygwin evaluates
...
def get_xpygwin_alias():
    """
    Determine the correct invocation to launch pygwin the same way the
    current session was launched.

    Always returns a list, so the result can be concatenated with other
    argv lists (e.g. ``['tmux', 'new-session'] + get_xpygwin_alias()``).

    For an entry-point launch (e.g. ``/usr/local/bin/pygwin``) the value of
    ``sys.argv[0]`` is already a runnable absolute path, so the result is
    a single-element list.

    For a "from source" launch via ``python -m pygwin`` (``sys.argv[0]``
    basename is ``__main__.py``) a naive ``[sys.executable, "-m", "pygwin"]``
    would be CWD-dependent: ``python -m pygwin`` resolves the package via
    ``sys.path``, which has the *current* working directory at position 0.
    Running ``xpygwin`` from outside the source repo would silently pick
    whatever ``import pygwin`` resolves to in ``site-packages`` (or raise
    ``ModuleNotFoundError``), which is almost never what the user wants.

    Instead, compute the parent directory of the source ``pygwin`` package
    ...
    importing ``pygwin.main``. This makes the alias resolve to the same
    ...
    if os.path.basename(current_pygwin) != "__main__.py":
        # Entry-point case: sys.argv[0] is an absolute path to the pygwin
        # launcher and is already runnable as-is.
        return [current_pygwin]

    # Source case: __main__.py lives inside the pygwin/ package, whose
    # parent directory is the one we need on sys.path for
    # ``from pygwin.main import main`` to pick up the source version.
    pkg_parent = os.path.dirname(os.path.dirname(os.path.abspath(current_pygwin)))
    bootstrap = (
        f"import sys; sys.path.insert(0, {pkg_parent!r}); "
        f"from pygwin.main import main; main()"
    )
...
def get_xpip_alias():
    """
    Determines the correct invocation to get pygwin's pip
    """
    ...
    try:
        if ON_WINDOWS:
            ...
            # In AppImage `sys.executable` is equal to path to pygwin.AppImage file and the real python executable is in $_
            return [
...
def _find_cmd_exe() -> str:
    ...
    indicate Pygwin (or other shell) as the default shell. (#5701)
    """
...
"""Built-in commands of ``cmd.exe`` that pygwin borrows on Windows."""
...
def win_sudo(args):
    ...
    :func:`pygwin.environ.locate_binary` and re-launches it through
...
def make_default_aliases():
    ...
    default_aliases = {
        ...
        "@lines": SpecAttrDecoratorAlias(
            ...
            "Command decorator. Return output as list of lines. See also $PYGWIN_SUBPROC_OUTPUT_FORMAT.",
            name="@lines",
        ),
        "@stream": SpecAttrDecoratorAlias(
            {"output_format": "stream_lines"},
            "Command decorator. Return output as stream of lines. See also $PYGWIN_SUBPROC_OUTPUT_FORMAT.",
            name="@stream",
        ),
    ...
    if ON_WINDOWS:
        ...
        if ON_ANACONDA or shutil.which("conda", path=XSH.env.get_detyped("PATH")):
            # ON_ANACONDA only fires when pygwin itself is installed inside the
            # conda env (sys.prefix has conda-meta/). Pip-installed pygwin +
            ...
            # `conda init pygwin` on Windows is broken — see pygwin/pygwin#3676.
