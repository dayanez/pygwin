"""Token-stream based formatting engine for pygwin source.

Why tokens, not AST
-------------------
pygwin has no pygwin-specific AST nodes — ``$(ls)`` parses to a regular
``Call(__pygwin__.subproc_capture_stdout, ...)``, ``${VAR}`` to a
``Subscript`` of ``__pygwin__.env``, and so on. Round-tripping through the
AST would lose the original syntactic form (was it ``$(...)`` or
``$[...]``?), drop comments, and erase the user's line breaks. Working
on the raw token stream from :mod:`pygwin.parsers.tokenize` keeps every
...
* preserves all pygwin-specific syntax verbatim — ``$()``, ``!()``,
  ``${...}``, ``$[]``, ``![]``, ``@()``, ``@$()``, IO redirects,
...
# Bracket-like openers (Python + pygwin subprocess).
_OPENERS = frozenset(("(", "[", "{", "$(", "$[", "${", "!(", "![", "@(", "@!(", "@$("))
_CLOSERS = frozenset((")", "]", "}"))
...
def format_source(src: str, *, indent: str = DEFAULT_INDENT) -> str:
    """Return ``src`` reformatted as pygwin source code.

    Parameters
    ----------
    src
        The pygwin source code to format.
    indent
...
class _Formatter:
    ...
    def _space_between(self, prev, cur) -> str:
        ...
        # pygwin tokenizer reports unreliable (often inverted) source
    ...
    def _render_token(self, tok) -> str:
        ...
        if tok.type == FSTRING_MIDDLE:
            ...
            # original characters and round-trip is exact. pygwin's
    ...
    def _is_subproc_statement(self, tokens: list, start_idx: int) -> bool:
        ...
        The pygwin parser performs the real subprocess-vs-Python decision
    ...
    def _raw_between(self, prev, cur) -> str:
        ...
        ``ERRORTOKEN`` pygwin emits in subprocess context) report an
    ...
    def _fstring_span_reliable(self, tok) -> bool:
        ...
        catches the pygwin multi-line ``FSTRING_MIDDLE`` quirk."""
        s_line, s_col = tok.start
...
def _format_comment(text: str) -> str:
    ...
    The only adjustment is an ``lstrip``: pygwin's tokenizer
