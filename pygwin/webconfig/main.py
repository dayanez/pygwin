...
from pygwin.webconfig.file_writes import insert_into_pygwinrc
from pygwin.webconfig.routes import Routes

RENDERERS: list[tp.Callable] = []


class PygwinConfigHTTPRequestHandler(server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        pprint(config)
        fname = insert_into_pygwinrc(config)
        print("Wrote out to " + fname)
        self._send(b"received post request:<br>" + post_body)


def bind_server_to(
    port: int = 8421, handler_cls=PygwinConfigHTTPRequestHandler, browser=False
):
...
if __name__ == "__main__":
    # watchexec -r -e py -- python -m pygwin.webconfig --no-browser
    main()
