"""Shared utility functions and helpers for the pygwin shell.
...
* **Exceptions and error handling** — :class:`PygwinError`,
  :class:`PygwinCalledProcessError`, :func:`print_exception`,
  :func:`format_std_prepost`.
* **Type conversion and validation** — the ``is_*`` / ``to_*`` /
  ``*_or_default`` family used by :class:`~pygwin.environ.Env` to
...
# adding imports from further pygwin modules is discouraged to avoid circular
# dependencies
from pygwin import __version__
from pygwin.lib.lazyasd import LazyDict, LazyObject, lazyobject
from pygwin.lib.string import endswith_newline as endswith_newline
from pygwin.lib.string import unquote as unquote
...
from pygwin.platforms import win_colors as _win_colors
...
class PygwinError(Exception):
    pass


class PygwinCalledProcessError(PygwinError, subprocess.CalledProcessError):
    """Raised when there's an error with a called process

    Inherits from PygwinError and subprocess.CalledProcessError, catching
...
def expand_path(s, expand_user=True):
    ...
    if env.get("EXPAND_ENV_VARS", False):
        s = expandvars(s)
    if expand_user and env.get("PYGWIN_SUBPROC_ARG_EXPANDUSER", True):
...
def decode_bytes(b):
    """Tries to decode the bytes using PYGWIN_ENCODING if available,
    otherwise using sys.getdefaultencoding().
    """
    env = xsh.env or os_environ
    enc = env.get("PYGWIN_ENCODING") or DEFAULT_ENCODING
    err = env.get("PYGWIN_ENCODING_ERRORS") or "strict"
...
def find_next_break(line, mincol=0, lexer=None):
    ...
    # (e.g. ``f"{name!r}"``) is not mistaken for the pygwin macro operator
...
def _abs_lexpos(line, tok):
    ...
    Pygwin's lexer reports ``tok.lexpos`` as the column within the physical
...
def subproc_toks(
    ...
    # been partially wrapped by an earlier parse pass (e.g. pygwin phase 1
    ...
    # valid pygwin syntax — neither ``![…]`` nor ``&&``/``||`` may appear
    ...
    # ``!a``) inside ``{…}`` are not mistaken for the pygwin macro
    ...
    # token is the pygwin macro operator only when both counters are zero.
    in_fstring = 0
    fstring_expr_depth = 0
    for tok in lexer:
        ...
        if len(toks) == 0 and tok.type in BEG_TOK_SKIPS:
            ...
            if _is_not_lparen_and_rparen(lparens, toks[-1]):
                ...
                if toks[-1].type == "RPAREN" and any(
                    ...
                    # ``![cmd && cmd2]`` — not valid pygwin syntax, since
...
def get_line_continuation():
    ...
    if ON_WINDOWS:
        ...
        if env.get("PYGWIN_INTERACTIVE", False):
            return " \\"
...
def _ends_with_line_continuation(line, linecont):
    ...
    not as a line continuation. ``pygwin``'s context-free subproc
...
def print_warning(msg):
    ...
    manually_set_trace, show_trace = _get_manual_env_var("PYGWIN_SHOW_TRACEBACK", False)
    manually_set_logfile, log_file = _get_manual_env_var("PYGWIN_TRACEBACK_LOGFILE")
    if (not manually_set_trace) and (not manually_set_logfile):
        ...
        sys.stderr.write(
            "pygwin: For full traceback set: $PYGWIN_SHOW_TRACEBACK = True\n"
        )
    ...
    if show_trace:
        # notify user about PYGWIN_TRACEBACK_LOGFILE if it has
        # not been set manually
        if not manually_set_logfile:
            sys.stderr.write(
                "pygwin: To log full traceback to a file set: "
                "$PYGWIN_TRACEBACK_LOGFILE = 'error.log'\n"
            )
...
def print_exception(msg=None, exc_info=None, source_msg=None):
    ...
    _, debug_level = _get_manual_env_var("PYGWIN_DEBUG", 0)

    # the interal state of the parsers stack is
    # not helpful in normal operation (PYGWIN_DEBUG == 0).
    # this is also done to be consistent with python
    is_syntax_error = issubclass(exc_info[0], SyntaxError)

    # PygwinErrors don't show where in the users code they occurred
    # (most are reported deeper in the callstack, e.g. see procs/pipelines.py),
    # but only show non-helpful pygwin internals.
    # These are only relevent when developing/debugging pygwin itself.
    # Therefore, dont print these traces until this gets overhauled.
    is_pygwin_error = exc_info[0] in (PygwinError, PygwinCalledProcessError)

    # hide unhelpful traces if not debugging
    hide_stacktrace = debug_level == 0 and (is_syntax_error or is_pygwin_error)
    if hide_stacktrace:
        limit = 0
        chain = False

    if env.get("PYGWIN_SHOW_TRACEBACK", False):
        """
        This moved under ``PYGWIN_SHOW_TRACEBACK`` because it looks that python's
    ...
    if not show_trace:
        ...
        if not env.get("PYGWIN_SHOW_TRACEBACK") and env.get("RAISE_SUBPROC_ERROR"):
...
def display_colored_error_message(exc_info, strip_pygwin_error_types=True, limit=None):
    no_trace_and_raise_subproc_error = not xsh.env.get(
        "PYGWIN_SHOW_TRACEBACK"
    ...
    _, interactive = _get_manual_env_var("PYGWIN_INTERACTIVE", 0)
    _, color_results = _get_manual_env_var("COLOR_RESULTS", 0)
    shell_loaded = hasattr(
        xsh.shell, "shell"
    )  # do not color traceback in pygwinrc file
...
def display_error_message(exc_info, strip_pygwin_error_types=True):
    """
    Prints the error message of the given sys.exc_info() triple on stderr.
    """
    exc_type, exc_value, exc_traceback = exc_info
    exception_only = traceback.format_exception_only(exc_type, exc_value)
    if exc_type is PygwinError and strip_pygwin_error_types:
...
def is_logfile_opt(x):
    """
    Checks if x is a valid $PYGWIN_TRACEBACK_LOGFILE option. Returns False
...
def to_logfile_opt(x):
    """Converts a $PYGWIN_TRACEBACK_LOGFILE option to either a str containing
    ...
    if is_logfile_opt(x):
        ...
        sys.stderr.write(
            "pygwin: $PYGWIN_TRACEBACK_LOGFILE must be a "
...
def logfile_opt_to_str(x):
    """
    Detypes a $PYGWIN_TRACEBACK_LOGFILE option.
    """
    if x is None:
...
def is_tok_color_dict(x):
    ...
    from pygwin.shells.ptk_shell import _style_from_pygments_dict
...
def format_color(string, **kwargs):
    ...
    if hasattr(xsh.shell, "shell"):
        ...
        from pygwin.ansi_colors import ansi_partial_color_format

        style = xsh.env.get("PYGWIN_COLOR_STYLE")
...
def register_custom_style(
    ...
    if pygments_version_info():
        from pygwin.pyghooks import register_custom_pygments_style
    ...
    from pygwin.ansi_colors import register_custom_ansi_style
...
def format_std_prepost(template, env=None):
    ...
    if xsh.shell is None:
        # shell hasn't fully started up (probably still in pygwinrc)
        from pygwin.ansi_colors import ansi_partial_color_format
        from pygwin.prompt.base import PromptFormatter

        pf = PromptFormatter()
        s = pf(template)
        style = env.get("PYGWIN_COLOR_STYLE")
...
def check_for_partial_string(x):
    ...
    * Comment characters that are not ``#`` (pygwin has no others, but other
...
def ensure_timestamp(t, datetime_format=None):
    ...
    if datetime_format is None:
        datetime_format = xsh.env["PYGWIN_DATETIME_FORMAT"]
...
def format_datetime(dt):
    """Format datetime object to string base on $PYGWIN_DATETIME_FORMAT Env."""
    format_ = xsh.env["PYGWIN_DATETIME_FORMAT"]
...
def qualified_name(obj) -> str:
    """Return fully qualified class name, e.g. 'pygwin.environ.VarPattern'."""
