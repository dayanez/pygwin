...
from pygwin.lib.lazyasd import LazyBool, lazybool, lazyobject

# do not import any pygwin-modules here to avoid circular dependencies
...
"""Minimum version of prompt-toolkit supported by Pygwin"""
...
def windows_expanduser(path):
    """A Windows-specific expanduser() function for pygwin. This is needed
...
def windows_bash_command():
    ...
    if bash_on_path:
        try:
            ...
            # which can't be used by pygwin foreign-shell/completer
...
def bash_command():
    ...
    if (bc := os.getenv("PYGWIN_BASH_PATH_OVERRIDE", None)) is not None:
...
def BASH_COMPLETIONS_DEFAULT():
    ...
    if ON_LINUX or ON_CYGWIN or ON_MSYS or ON_TERMUX:
        ...
        # (``[[ "$-" =~ i ]] || return 0``) — and pygwin's bridge runs
