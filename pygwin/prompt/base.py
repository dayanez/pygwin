...
class PromptFormatter:
    ...
    def __call__(self, template=DEFAULT_PROMPT, fields=None, **kwargs) -> str:
        """Formats a pygwin prompt template string."""
...
def _failover_template_format(template):
    if callable(template):
        try:
            ...
            # in user's pygwinrc should not crash pygwin
            return template()
        except Exception:
            xt.print_exception()
            return "$ "
...
class PromptFields(tp.MutableMapping[str, "FieldType"]):
    ...
    def __init__(self, xsh: "PygwinSession", init=True):
    ...
    def load_initial(self):
        ...
        from pygwin.prompt.cwd import (
            _collapsed_pwd,
            _dynamically_collapsed_pwd,
            _replace_home_cwd,
        )
        from pygwin.prompt.env import env_name, vte_new_tab_cwd
        from pygwin.prompt.job import CurrentJobField
        from pygwin.prompt.times import _localtime
        from pygwin.prompt.vc import branch_bg_color, branch_color, current_branch
