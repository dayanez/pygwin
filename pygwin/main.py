...
from pygwin.codecache import run_code_with_cache, run_script_with_cache
...
from pygwin.imphooks import install_import_hooks
...
from pygwin.timings import setup_timings
...
.. code-block:: pygwin
    ...
    def _event_save_session_log(exit_code, **kw):
        import datetime
        with open("/tmp/pygwin_sessions.log", "a") as f:
...
.. code-block:: pygwin
    ...
    def _event_show_tip(**kw):
        import random
        tips = ["Use Tab for completion", "Try 'xonfig' to configure pygwin"]
...
def get_current_pygwin():
    """Return the path used to invoke the current pygwin session.

    This is a thin wrapper around ``sys.argv[0]`` so that callers can share
    a single source of truth for "how was this pygwin started".

    Examples
    --------
    - Entry point: ``/usr/bin/pygwin``.
    - Source via ``python -m pygwin``: ``/path/to/pygwin/__main__.py``
    """
    return sys.argv[0]
...
def _handle_sig_ttin_ttou(n, f):
    ...
    Under normal operation pygwin is the foreground process group of
    ...
    1. **PEP 475 (``os.read`` retry) — triggered by SIGTTIN.**
       ...
       https://peps.python.org/pep-0475/ for the full list. If pygwin
    ...
    pygwin is already in a degraded "lost foreground" state, and the
...
def _acquire_controlling_terminal():
    """Put pygwin into the foreground process group of its controlling TTY.
    ...
    TTY. pygwin historically did not do this — it relied on the launching
    ...
    * Build systems, CI runners, IDE integrated terminals and service
      managers (e.g. ``systemd --user``) that don't wire pygwin up as the
      foreground process of the PTY they attach.
    * Nested containers that reuse the outer PTY.

    In those cases pygwin is technically a background process. Every TTY
    ...
    The fix
    -------
    Perform the same handshake bash does, very early in pygwin's lifetime:

    1. Find a TTY file descriptor (stderr, same choice pygwin already uses
       for :func:`pygwin.procs.jobs.give_terminal_to`).
    ...
    Environment
    -----------
    Set ``PYGWIN_NO_FG_TAKEOVER=1`` in the parent environment to disable
    the handshake entirely. This is an escape hatch for rare cases where
    the takeover conflicts with a parent process that expects pygwin to
    stay in its original process group.

    Returns
    -------
    bool
        ``True`` if pygwin is now (or already was) the foreground process
    ...
    * Original xonsh-flatpak wrapper that documented the failure modes
    ...
    if ON_WINDOWS:
        return False
    if os.environ.get("PYGWIN_NO_FG_TAKEOVER"):
        return False
    ...
    if not hasattr(signal, "pthread_sigmask"):
        return False
    # Use stderr as the TTY handle. pygwin already uses FD 2 for
    # give_terminal_to in pipeline management (pygwin/procs/jobs.py), so
    ...
    try:
        ...
        if current_fg == my_pgid and my_pgid > 0:
            ...
            # :func:`pygwin.procs.jobs.give_terminal_to` would fail with
            # ESRCH (tcsetpgrp(3):
            #   https://man7.org/linux/man-pages/man3/tcsetpgrp.3.html),
...
def _release_controlling_terminal():
    ...
    actually took over the TTY. A no-op in every other case: if pygwin was
    ...
    shell (bash, zsh, another pygwin, …) usually calls ``wait`` on us and
    then expects to read from the terminal. If pygwin leaves its own pgid
...
def _setup_ctrl_break():
    """Windows: make Ctrl+Break interrupt the foreground job, not kill pygwin.

    The Windows console delivers ``CTRL_BREAK_EVENT`` to *every* process
    attached to the console — pygwin and the subprocess it is waiting on
    alike. Python maps that event to ``SIGBREAK``, whose default disposition
    terminates the process, so with no handler installed pygwin itself is torn
    ...
    event directly and exits on its own; pygwin's wait then unblocks, the
    ...
    ``pygwin/procs/posix.py`` and ``pygwin/procs/proxies.py``).
    ...
    :func:`pygwin.built_ins.resetting_signal_handle`.
    """
...
def _setup_controlling_terminal():
    ...
    to take over the TTY via ``tcsetattr``. When pygwin is foreground
    *before* rc runs, every downstream TTY operation simply works; when
    ...
    during rc execution. It is also called from :func:`main_pygwin` as a
    ...
    the historical pygwin behavior (before this function existed,
    ``main_pygwin`` installed the same no-op handler unconditionally)
    and protects script-mode pygwin from being suspended by
    ...
    * PEP 475 (Retry system calls failing with EINTR):
    ...
    # handler is a trap — a background pygwin touching the TTY would
    # loop forever via PEP 475 retry on ``os.read`` (SIGTTIN path) or
    # via the application-level retry in
    # ``pygwin/shells/ptk_shell/__init__.py`` (SIGTTOU path). The
    ...
    if fg_acquired:
        ...
        # SIGTTIN/SIGTTOU every time pygwin touches the TTY, and a
...
def parser():
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "subcommands:\n"
            "  format          Format pygwin source files. "
            "Run `pygwin format --help` for options.\n"
            "  check           Check pygwin source files for syntax errors "
            "without running them. Run `pygwin check --help` for options.\n"
            "  lint            Lint pygwin source files for likely mistakes. "
            "Run `pygwin lint --help` for options.\n"
        ),
    )
    p.add_argument(
        "-h",
    ...
    p.add_argument(
        ...
        "errors. Use the `pygwin check` subcommand for more options "
    ...
    p.add_argument(
        "--rc",
        help="The pygwinrc files to load, these may be either pygwin "
        "files or directories containing pygwin files",
        dest="rc",
        nargs="+",
        type=path_argument,
        default=None,
    ...
    p.add_argument(
        "--no-rc",
        help="Do not load any pygwin RC files. Argument --rc will "
    ...
    p.add_argument(
        "--save-origin-env",
        help="Save origin environment variables before running pygwin. Use --load-origin-env to run pygwin with saved origin environment.",
    ...
    p.add_argument(
        "--load-origin-env",
        help="Load origin environment variables that were saved before running pygwin by using --save-origin-env",
...
def _pprint_displayhook(value):
    ...
    # displayhook does. See https://github.com/pygwin/pygwin/issues/6503.
    if HAS_PYGMENTS and env.get("COLOR_RESULTS") and "\x1b" not in printed_val:
        tokens = list(pygments.lex(printed_val, lexer=pyghooks.PygwinLexer()))
...
def _get_rc_files(shell_kwargs: dict, args, env):
    ...
    if rc_cli:
        ...
        # file, and nothing else (ignore both PYGWINRC and PYGWINRC_DIR)
        rc = tuple(r for r in rc_cli if os.path.isfile(r))
        rcd = tuple(r for r in rc_cli if os.path.isdir(r))
        return rc, rcd

    # otherwise, get the RC files from PYGWINRC, and RC dirs from PYGWINRC_DIR
    rc = env.get("PYGWINRC")
    rcd = env.get("PYGWINRC_DIR")

    if not env.get("PYGWIN_INTERACTIVE", False):
        """
        Home based ``~/.pygwinrc`` file has special meaning and history. The ecosystem around shells treats this kind of files
        ...
        """
        home_pygwinrc = get_home_pygwinrc_path()
        rc = tuple(c for c in rc if c != home_pygwinrc)
...
def start_services(shell_kwargs, args, pre_env=None):
    ...
    debug = to_bool_or_int(os.getenv("PYGWIN_DEBUG", "0"))
    ...
    events.on_timingprobe.fire(name="pre_pygwin_session_load")
    XSH.load(
        ctx=ctx,
        execer=execer,
    ...
    events.on_timingprobe.fire(name="post_pygwin_session_load")
...
def premain(argv=None):
    """Setup for main pygwin entry point. Returns parsed arguments."""
    if argv is None:
        argv = sys.argv[1:]
    setup_timings(argv)
    setproctitle = get_setproctitle()
    if setproctitle is not None:
        setproctitle(" ".join(["pygwin"] + argv))
    args = parser.parse_args(argv)
    if args.help:
        parser.print_help()
        parser.exit()
    if args.no_execute:
        from pygwin.checker.cli import check_no_execute
    ...
    if args.command is not None:
        args.mode = PygwinMode.single_command
        shell_kwargs["shell_type"] = "none"
        pygwin_mode = "single_command"
    elif args.file is not None:
        args.mode = PygwinMode.script_from_file
        shell_kwargs["shell_type"] = "none"
        pygwin_mode = "script_from_file"
    elif not sys.stdin.isatty() and not args.force_interactive:
        args.mode = PygwinMode.script_from_stdin
        shell_kwargs["shell_type"] = "none"
        pygwin_mode = "script_from_stdin"
    else:
        args.mode = PygwinMode.interactive
        shell_kwargs["completer"] = True
        shell_kwargs["login"] = True
        pygwin_mode = "interactive"

    pre_env = {
        "PYGWIN_LOGIN": shell_kwargs["login"],
        "PYGWIN_INTERACTIVE": args.force_interactive
        or (args.mode == PygwinMode.interactive),
        "PYGWIN_MODE": pygwin_mode,
    }
    ...
    if args.defines is not None:
        for x in args.defines:
            ...
            if len(var) == 2:
                ...
                elif os_environ.get("PYGWIN_DEBUG", "0") != "0":
                    print(
    ...
    pre_env["COLOR_RESULTS"] = os.getenv(
        "COLOR_RESULTS", str(pre_env["PYGWIN_INTERACTIVE"])
    )
...
def _failback_to_other_shells(args, err):
    ...
    if hasattr(args, "mode") and args.mode != PygwinMode.interactive:
        raise err

    foreign_shell = None

    # look first in users login shell $SHELL.
    # use real os.environ, in case Pygwin hasn't initialized yet
    ...
    if not foreign_shell:
        excluded_list = ["pygwin", "pygwin", "screen"]
    ...
    if foreign_shell:
        traceback.print_exc()
        print("Pygwin encountered an issue during launch.", file=sys.stderr)
        print(
            "Please report to https://github.com/pygwin/pygwin/issues", file=sys.stderr
        )
...
def main(argv=None):
    ...
    if argv and argv[0] == "format":
        from pygwin.formatter.cli import main as format_main

        sys.exit(format_main(argv[1:]))

    if argv and argv[0] == "check":
        from pygwin.checker.cli import main as check_main

        sys.exit(check_main(argv[1:]))

    if argv and argv[0] == "lint":
        from pygwin.linter.cli import main as lint_main
    ...
    # the race between their own TTY manipulation and pygwin's
    # :func:`pygwin.procs.jobs.give_terminal_to` never has to open.
    # See :func:`_setup_controlling_terminal` for the full rationale.
    _setup_controlling_terminal()
    args = None
    try:
        args = premain(argv)
        sys.exit(main_pygwin(args))
    except Exception as err:
        _failback_to_other_shells(args, err)


def main_pygwin(args):
    """Main entry point for pygwin cli."""
    ...
    if shell and not env["PYGWIN_INTERACTIVE"]:
    ...
    # if this does not change, no exceptions were thrown. Otherwise, print a traceback that does not expose pygwin internals
    exc_info = None, None, None

    try:
        if args.mode == PygwinMode.interactive:
            # enter the shell

            # Setted again here because it is possible to call main_pygwin() without calling premain(), namely in the tests.
            env["PYGWIN_INTERACTIVE"] = True

            ignore_sigtstp()
            if (
                env["PYGWIN_INTERACTIVE"]
                and not env["PYGWIN_SUPPRESS_WELCOME"]
                and sys.stdin.isatty()  # In case the interactive mode is forced but no tty (input from pipe).
                and not any(os.path.isfile(i) for i in env["PYGWINRC"])
                and not any(os.path.isdir(i) for i in env["PYGWINRC_DIR"])
            ):
        ...
        elif args.mode == PygwinMode.single_command:
        ...
        elif args.mode == PygwinMode.script_from_file:
            # run a script contained in a file
            path = os.path.abspath(os.path.expanduser(args.file))
            if os.path.isdir(path):
                print(f"pygwin: {args.file}: Is a directory.")
                exit_code = 1
            elif os.path.exists(path):
                sys.argv = [args.file] + args.args
                env.update(make_args_env())  # $ARGS is not sys.argv
                env["PYGWIN_SOURCE"] = path
                ...
                print(f"pygwin: {args.file}: No such file.")
                exit_code = 1
        elif args.mode == PygwinMode.script_from_stdin:
        ...
        exit_code = 1
        debug_level = env.get("PYGWIN_DEBUG", 0)
        if debug_level == 0:
...
def postmain(args=None):
    """Teardown for main pygwin entry point, accepts parsed arguments."""
    XSH.unload()
    XSH.shell = None
...
def setup(
    ...
    """Starts up a new pygwin shell. Calling this in function in another
    packages ``__init__.py`` will allow pygwin to be fully used in the
    package in headless or headed mode. This function is primarily indended to
    make starting up pygwin for 3rd party packages easier.
    ...
    ctx : dict-like or None, optional
        The pygwin context to start with. If None, an empty dictionary
        is provided.
