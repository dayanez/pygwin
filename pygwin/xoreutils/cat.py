"""Implements a cat command for pygwin."""
...
from pygwin.xoreutils.util import arg_handler, run_alias
...
def _cat_single_file(opts, fname, stdin, out, err, line_count=1, read_timeout=0.1):
    ...
    enc_errors = env.get("PYGWIN_ENCODING_ERRORS")
    read_size = 0
    file_size = fobj = None
    if fname == "-":
    ...
    try:
        while file_size is None or read_size < file_size:
            try:
                ...
                errors += 1
                print("pygwin:", e, flush=True, file=out)
                if errors > 10:
                    break
    finally:
        if fobj is not None:
...
CAT_HELP_STR = """This version of cat was written in Python for the pygwin project: http://xon.sh
