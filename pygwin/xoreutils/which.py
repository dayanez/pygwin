...
from pygwin.xoreutils import _which
...
def _which_create_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="all",
        help="Show all matches in globals, pygwin.aliases, $PATH",
    )
    parser.add_argument(
        "-s",
        "--skip-alias",
        action="store_true",
        help="Do not search in pygwin.aliases",
        dest="skip",
    )
    parser.add_argument(
        "-V",
        ...
        help="Display the version of the python which module used by pygwin",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def print_alias(arg, stdout, verbose=False):
    ...
    if not verbose:
        ...
        elif isinstance(alias, pygwin.aliases.ExecAlias):
        ...
        if callable(alias) and not isinstance(alias, pygwin.aliases.ExecAlias):
            XSH.superhelp(alias)


def which(args, stdin=None, stdout=None, stderr=None, spec=None):
    """
    Checks if each arguments is a pygwin aliases, then if it's an executable,
    then finally return an error code equal to the number of misses.
    If '-a' flag is passed, run both to return both `pygwin` match and
    `which` match.
    """
