...
from pygwin.cli_utils import ArgParserAlias
from pygwin.platform_info import IN_APPIMAGE
from pygwin.procs.executables import locate_executable, locate_relative_path
from pygwin.tools import print_color


def _get_version(binary, arg_ver="--version"):
    """Return ``(version_string, ok)`` for a python/pygwin/pip binary.
    ...
    ``XSH.subproc_captured_stdout``. The pygwin pipeline framework
    (``pygwin/procs/pipelines.py``) catches spawn errors and calls
...
def _is_executable_file(path):
    ...
    * Windows , :func:`os.access` with ``X_OK`` inspects the file
    ...
    them red would be a false positive for ``xpygwin`` when the current
    session was started via ``python -m pygwin``.
    """
    if not path:
        return False
    try:
...
class Resolved:
    ...
    :func:`locate_executable`, :func:`get_current_pygwin`,
...
class XContext:
    ...
    # ---- session: what the running pygwin process actually uses --------

    @_cached_method
    def get_session_xpygwin(self) -> Resolved:
        """Return the path to the currently running pygwin interpreter."""
        # Local import: ``pygwin.main`` pulls in heavy modules.
        from pygwin.main import get_current_pygwin

        original, resolved, bad = _resolve_path(get_current_pygwin(), self._resolve)
        return Resolved(path=original, resolved=resolved, bad=bad)

    @_cached_method
    def get_session_xpython(self) -> Resolved:
        """Return the python interpreter that's running this pygwin.
    ...
    @_cached_method
    def get_commands_pygwin(self) -> Resolved:
        """Return the ``pygwin`` binary that ``$PATH`` resolves to."""
        return self._get_command("pygwin")
...
def xcontext_main(no_resolve: bool = False, as_json: bool = False, _stdout=None):
    """Report information about the current pygwin environment.
    ...
    as_json : -j, --json
        ...
        (e.g. ``xpygwin``) , and, **only when the resolved path differs
        from the input**, an additional sibling key with a ``_resolved``
        suffix (e.g. ``xpygwin_resolved``). This mirrors the colored
    ...
    session_xpygwin = xc.get_session_xpygwin()
    session_xpython = xc.get_session_xpython()
    session_xpip = xc.get_session_xpip()
    cmd_pygwin = xc.get_commands_pygwin()
    ...
    # Color tokens: section headers are purple; within a family (pygwin/xpygwin,
    ...
    pygwin_color = GREEN if session_xpygwin.resolved == cmd_pygwin.resolved else BLUE
    ...
    print_color(f"{PURPLE}[Current pygwin session]{RESET}", file=stdout)
    _print_pair("xpygwin", session_xpygwin)
