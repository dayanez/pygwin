"""Debug utilities for a running pygwin session.

Provides :class:`PygwinDebug`, which is attached to the session as
``__pygwin__.debug`` and exposed to user code through the ``@.`` interface
as ``@.debug``. The main entry point is :meth:`PygwinDebug.breakpoint`.
"""
...
class PygwinDebugQuit(Exception):
    """Raised from the ``eval`` REPL when the user types ``exit``/``quit``/``q``.

    Propagates out of :meth:`PygwinDebug.breakpoint` so that execution at
...
def is_breakpoint_engine(x):
    """Enumerated values of $PYGWIN_DEBUG_BREAKPOINT_ENGINE."""
    return isinstance(x, str) and x in CANONIC_BREAKPOINT_ENGINES


def to_breakpoint_engine(x):
    """Convert user input to value of $PYGWIN_DEBUG_BREAKPOINT_ENGINE."""
    y = str(x).casefold()
    if y not in CANONIC_BREAKPOINT_ENGINES:
        warnings.warn(
            f"'{x}' is not valid for $PYGWIN_DEBUG_BREAKPOINT_ENGINE, "
...
class PygwinDebug:
    """Debug helpers for a running pygwin session.

    Accessible from pygwin code as ``@.debug``.
    ...
    def breakpoint(self, engine: str = "auto", *, frame=None) -> None:
        ...
        engine : str, optional
            ...
            ``"auto"`` (the default), ``$PYGWIN_DEBUG_BREAKPOINT_ENGINE``
            is consulted first; if it is also ``"auto"``, the first
            available engine is chosen in priority order: ``pdbp`` ->
            ``ipdb`` -> ``pdb`` -> ``execer`` (if a pygwin session is
            loaded) -> ``eval`` (always).

            The ``"execer"`` engine starts a REPL at the caller's
            frame, backed by the session's :class:`~pygwin.execer.Execer`
            (prompt ``"execer> "``) — pygwin syntax such as
            ...
            ``"(xdebug) "``) — it has no dependency on a pygwin session.

            In both REPLs, type ``c``/``cont``/``continue`` (or hit
            EOF/Ctrl-C) to resume execution, or ``exit``/``quit``/``q``
            to abort — the latter raises :class:`PygwinDebugQuit`.
    ...
    _HINTS = {
        "pdb": "[pygwin-debug] pdb: 'c' continue | 'q' quit",
        "ipdb": "[pygwin-debug] ipdb: 'c' continue | 'q' quit",
        "pdbp": "[pygwin-debug] pdbp: 'c' continue | 'q' quit",
        "execer": (
            "[pygwin-debug] execer REPL: 'c'/'continue' resume | 'q'/'quit'/'exit' abort"
        ),
        "eval": (
            "[pygwin-debug] eval REPL: 'c'/'continue' resume | 'q'/'quit'/'exit' abort"
        ),
    }
    ...
    def replace_builtin_breakpoint(self, engine: str = "auto") -> None:
        ...
        statement — whether in pygwin code or in plain Python modules
        loaded from pygwin — drops into the engine chosen by this
        object. Call it from ``~/.pygwinrc`` to make the integration
        persistent.
    ...
    def _has_execer() -> bool:
        xsh = getattr(builtins, "__pygwin__", None)
    ...
    def _env_default() -> str:
        ...
        if env is None:
            return "auto"
        value = env.get("PYGWIN_DEBUG_BREAKPOINT_ENGINE", "auto")
    ...
    def _execer_repl(self, frame) -> None:
        ...
        if execer is None:
            raise RuntimeError(
                "The 'execer' breakpoint engine requires an active pygwin "
                "session with an Execer attached. Use engine='eval' for a "
                "plain-Python REPL, or run inside a pygwin session."
            )
    ...
    def _interactive_repl(
        ...
        print(f"[pygwin-debug] {banner_backend} REPL at {location}")
        ...
        while True:
            ...
            except (SystemExit, PygwinDebugQuit):
                raise
