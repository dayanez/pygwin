"""Environment for the pygwin shell."""
...
import pygwin.prompt.base as prompt
...
from pygwin.codecache import run_script_with_cache
from pygwin.debug import is_breakpoint_engine, to_breakpoint_engine
from pygwin.dirstack import _get_cwd
from pygwin.events import events
from pygwin.lib.lazyasd import LazyBool, lazyobject
...
events.doc(
    "on_pre_spec_run_ls",
    """
on_pre_spec_run_ls(spec: pygwin.built_ins.SubprocSpec) -> None
...
class LsColors(cabc.MutableMapping):
    """Helps convert to/from ``$LS_COLORS`` format, respecting the pygwin color style.
    ...
    @property
    def style_name(self):
        """Current PYGWIN_COLOR_STYLE value"""
        env = getattr(XSH, "env", {}) or {}
        env_style_name = env.get("PYGWIN_COLOR_STYLE", "default")
    ...
    @property
    def style(self):
        """The ANSI color style for the current PYGWIN_COLOR_STYLE"""
    ...
    def fromstring(cls, s):
        ...
        # pygwin names should be done from defaults
        ...
        for item in s.split(":"):
            ...
            if not eq:
                ...
                try:
                    ...
                    print("pygwin:warning:" + str(e), file=sys.stderr)
                    ini_dict[key] = ("RESET",)
        return cls(ini_dict)
...
class VarPattern:
    ...
    Example::
        ...
        $PYGWIN_ENV_PATTERN_PATH = VarPattern(r"\\w*PATH$", "env_path")
    """
...
def default_value(f):
    ...
    f._pygwin_callable_default = True
    return f


def is_callable_default(x):
    """Checks if a value is a callable default."""
    return callable(x) and getattr(x, "_pygwin_callable_default", False)


DEFAULT_TITLE = "{current_job:{} | }{user}@{hostname}: {cwd} | pygwin"


@default_value
def pygwin_data_dir(env):
    """Ensures and returns the ``$PYGWIN_DATA_DIR``"""
    xdd = os.path.expanduser(
        os.getenv("PYGWIN_DATA_DIR") or os.path.join(env.get("XDG_DATA_HOME"), "pygwin")
    )
...
@default_value
def pygwin_cache_dir(env):
    """Ensures and returns the ``$PYGWIN_CACHE_DIR``"""
    xdd = os.path.expanduser(
        os.getenv("PYGWIN_CACHE_DIR")
        or os.path.join(env.get("XDG_CACHE_HOME"), "pygwin")
    )
    os.makedirs(xdd, exist_ok=True)
    return xdd
...
@default_value
def pygwin_config_dir(env):
    """``$XDG_CONFIG_HOME/pygwin``"""
    xcd = os.path.expanduser(
        os.getenv("PYGWIN_CONFIG_DIR")
        or os.path.join(env.get("XDG_CONFIG_HOME"), "pygwin")
    )
...
@default_value
def pygwin_sys_config_dir(env):
    """
    On Linux & macOS: ``'/etc/pygwin'``
    On Windows: ``'%ALLUSERSPROFILE%\\\\pygwin'``
    """
    if ON_WINDOWS:
        etc_path = os_environ["ALLUSERSPROFILE"]
    else:
        etc_path = "/etc"
    return os.path.join(etc_path, "pygwin")


def pygwinconfig(env):
    """Ensures and returns the ``$PYGWINCONFIG``"""
    xcd = env.get("PYGWIN_CONFIG_DIR")
...
@default_value
def default_pygwinrc(env) -> "tuple[str, ...]":
    """
    ``['$PYGWIN_SYS_CONFIG_DIR/pygwinrc', '$PYGWIN_CONFIG_DIR/rc.xsh', '~/.pygwinrc']``
    """

    dxrc = (
        os.path.join(pygwin_sys_config_dir(env), "pygwinrc"),
        os.path.join(pygwin_config_dir(env), "rc.xsh"),
        get_home_pygwinrc_path(),
    )
    # Check if old config file exists and issue warning
    old_config_filename = pygwinconfig(env)
    if os.path.isfile(old_config_filename):
        print(
            "WARNING! old style configuration ("
            + old_config_filename
            + ") is no longer supported. "
            + "Please migrate to pygwinrc."
        )
    return dxrc
...
def get_config_paths(env: "Env", name: str) -> tuple[str, ...]:
    paths: tuple[str, ...] = (
        os.path.join(pygwin_sys_config_dir(env), name),
        os.path.join(pygwin_config_dir(env), name),
    )
    if IN_FLATPAK:
        paths = (os.path.join("/app/etc/pygwin", name),) + paths
    return paths


@default_value
def default_pygwinrcdir(env):
    """``['$PYGWIN_SYS_CONFIG_DIR/rc.d', '$PYGWIN_CONFIG_DIR/rc.d']``\n"""
...
def default_completer_dirs(env):
    ...
    1. ``$PYGWIN_CONFIG_DIR/completions`` - user level completions
    2. ``$PYGWIN_SYS_CONFIG_DIR/completions`` - system level completions
    ...
    return [
        os.path.join(env["PYGWIN_CONFIG_DIR"], "completions"),
        os.path.join(env["PYGWIN_SYS_CONFIG_DIR"], "completions"),
        os.path.join(env["PYGWIN_DATA_DIR"], "generated_completions"),
    ] + [
        os.path.join(parent, "pygwin", "vendor_completions")
        for parent in env["XDG_DATA_DIRS"]
    ]


@default_value
def pygwin_append_newline(env):
    """Appends a newline if we are in interactive mode"""
    return env.get("PYGWIN_INTERACTIVE", False)
...
def default_prompt_fields(env):
    """``pygwin.prompt.base.PromptFields``"""
...
def _commands_cache_read_dir_once_default():
    """Compute the default for ``$PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE``.
...
class SystemSetting(Xettings):
    ...
    pygwin-specific — pygwin only provides sensible defaults and lets scripts
    ...
    HOSTTYPE = Var.with_default(
        ...
        doc="Automatically set to a string that fully describes the system type on which pygwin is executing.",
        type_str="str",
    )
    ...
    TERM = Var.no_default(
        ...
        "options:\n\n"
        "* Set this from the program that launches pygwin. On POSIX systems, \n"
        "  this can be performed by using env, e.g. \n"
        "  ``/usr/bin/env TERM=xterm-color pygwin`` or similar.\n"
        "* From the pygwin command line, namely ``pygwin -DTERM=xterm-color``.\n"
        '* In the config file with ``{"env": {"TERM": "xterm-color"}}``.\n'
        "* Lastly, in pygwinrc with ``$TERM``\n\n"
...
class GeneralSetting(Xettings):
    ...
    UPDATE_OS_ENVIRON = Var.with_default(
        ...
        "when the pygwin environment changes. The environment can be reset to "
        "the default value by calling ``__pygwin__.env.undo_replace_env()``",
    )
    PYGWIN_CONFIG_DIR = Var.with_default(
        pygwin_config_dir,
        "This is the location where pygwin user-level configuration information is stored.",
        type_str="str",
    )
    PYGWIN_ORIGIN_ENV_FILE = Var.no_default(
        "str",
        "The path to the file where environment variables are saved when "
        "running ``pygwin --save-origin-env``. When pygwin ``--load-origin-env`` "
        "is executed, this file is used as the basis for the environment. "
        "Thus, from an existing pygwin session, you can start a new one with "
        "the same environment variables that were used when launching the original session.",
    )
    PYGWIN_SYS_CONFIG_DIR = Var.with_default(
        pygwin_sys_config_dir,
        "This is the location where pygwin system-level configuration information is stored.",
        is_configurable=False,
        type_str="str",
    )
    PYGWINRC = Var.with_default(
        default_pygwinrc,
    ...
    PYGWINRC_DIR = Var.with_default(
        ...
        "are loaded after any files in PYGWINRC.",
        type_str="env_path",
    )
    PYGWIN_COLOR_STYLE = Var.with_default(
        "default",
        "Sets the color style for pygwin colors. This is a style name, not "
    ...
    PYGWIN_DEBUG = Var(
        ...
        bool_or_int_to_str,
        0,
        "Sets the pygwin debugging level. This may be an integer or a boolean. "
    ...
    PYGWIN_DEBUG_BREAKPOINT_ENGINE = Var(
    ...
    PYGWIN_DATA_DIR = Var.with_default(
        pygwin_data_dir,
        "This is the location where pygwin data files are stored, such as history, generated completers ...",
        doc_default="``$XDG_DATA_HOME/pygwin``",
        type_str="str",
    )
    PYGWIN_ENCODING = Var.with_default(
        DEFAULT_ENCODING,
        "This is the encoding that pygwin should use for subprocess operations.",
        doc_default="``sys.getdefaultencoding()``",
        type_str="str",
    )
    PYGWIN_ENCODING_ERRORS = Var.with_default(
        "surrogateescape",
    ...
    PYGWIN_INTERACTIVE = Var.with_default(
        True,
        "``True`` if pygwin is running interactively, and ``False`` otherwise. "
        "It's highly recommended to use this variable in your ``pygwinrc`` files "
        "to split the code execution for interactive and non-interactive modes.",
        is_configurable=False,
    )
    PYGWIN_LOGIN = Var.with_default(
        False,
        "``True`` if pygwin is running as a login shell, and ``False`` otherwise.",
        is_configurable=False,
    )
    PYGWIN_MODE = Var.with_default(
        default="interactive",  # In sync with ``main.py``.
        doc="A string value representing the current pygwin execution mode: "
        ...
        "you plan to ``source``, use ``$PYGWIN_INTERACTIVE`` as the flag instead.",
        type_str="str",
    )
    PYGWIN_SOURCE = Var.with_default(
        "",
        "When running a pygwin script, this variable contains the absolute path "
        "to the currently executing script's file.",
        is_configurable=False,
    )
    PYGWIN_STORE_STDIN = Var.with_default(
        False,
        "Whether or not to store the stdin that is supplied to the "
        "``!()`` and ``![]`` operators.",
    )
    PYGWIN_STYLE_OVERRIDES = Var(
        is_tok_color_dict,
        to_tok_color_dict,
        dict_to_str,
        {},
        ...
        "- ``pygments.token.Token`` - ``$PYGWIN_STYLE_OVERRIDES[Token.Keyword] = '#ff0000'``\n"
        "- pygments token name (string) - ``$PYGWIN_STYLE_OVERRIDES['Token.Keyword'] = '#ff0000'``\n"
        "- ptk style name (string) - ``$PYGWIN_STYLE_OVERRIDES['pygments.keyword'] = '#ff0000'``\n\n"
        "(The rules above all have the same effect.)",
    )
    PYGWIN_ENV_PATTERN_PATH = Var.with_default(
        VarPattern(r"\w*PATH$", "env_path"),
        "Pattern rule: env vars matching this regex are treated as env_path.",
        type_str="var_pattern",
    )
    PYGWIN_ENV_PATTERN_DIRS = Var.with_default(
...
class SubprocessSetting(Xettings):
    ...
    PYGWIN_SUBPROC_CMD_RAISE_ERROR = Var.with_default(
        False,
    ...
    PYGWIN_SUBPROC_CAPTURED_PRINT_STDERR = Var.with_default(
        False,
        "If ``True`` the stderr from captured subproc will be printed automatically.",
    )
    PYGWIN_BUILTINS_TO_CMD = Var.with_default(
        False,
    ...
    PYGWIN_SUBPROC_OUTPUT_FORMAT = Var.with_default(
        "stream_lines",
    ...
    PYGWIN_CAPTURE_ALWAYS = Var.with_default(
        False,
        "Try to capture output of commands run without explicit capturing.\n"
        "If True, pygwin will capture the output of commands run directly or in ``![]`` "
    ...
    THREAD_SUBPROCS = Var(
        ...
        bool_or_none_to_str,
        not ON_CYGWIN,
        "Note: The ``$PYGWIN_CAPTURE_ALWAYS`` variable introduces finer control "
        ...
        "If True, pygwin is able capture & store the stdin, stdout, and stderr"
        " of threadable subprocesses.\n"
        "The disadvantages are listed in ``$PYGWIN_CAPTURE_ALWAYS``.\n"
        ...
        "threading when loading pygwinrc files. This is done because Bash "
        "was automatically placing new pygwin instances in the background "
        "at startup when threadable subprocs were used. Please see "
        "https://github.com/xonsh/xonsh/pull/3705 for more information.\n",
    )
    PYGWIN_APPEND_NEWLINE = Var.with_default(
        pygwin_append_newline,
        "Append new line when a partial line is preserved in output.",
        doc_default="``$PYGWIN_INTERACTIVE``",
        type_str="bool",
    )
    PYGWIN_PROC_FREQUENCY = Var.with_default(
        1e-4,
        "The process frequency is the time that "
        "pygwin process threads sleep for while running command pipelines. "
        "The value has units of seconds [s].",
    )
    PYGWIN_SUBPROC_TRACE = Var(
        default=False,
...
class ErrorHandlingSetting(Xettings):
    ...
    PYGWIN_SHOW_TRACEBACK = Var.with_default(
        False,
    ...
    PYGWIN_TRACEBACK_LOGFILE = Var(
        is_logfile_opt,
        to_logfile_opt,
        logfile_opt_to_str,
        None,
        "Specifies a file to store the traceback log to, regardless of whether "
        "``PYGWIN_SHOW_TRACEBACK`` has been set. Its value must be a writable file "
...
class LangSetting(Xettings):
    ...
    PYGWIN_DATETIME_FORMAT = Var.with_default(
        "%Y-%m-%d %H:%M",
...
class CacheSetting(Xettings):
    ...
    PYGWIN_CACHE_SCRIPTS = Var.with_default(
        True,
        "Controls whether the code for scripts run from pygwin will be cached"
        " (``True``) or re-compiled each time (``False``).",
    )

    PYGWIN_CACHE_EVERYTHING = Var.with_default(
        False,
        "Controls whether all code (including code entered at the interactive"
        " prompt) will be cached.",
    )

    PYGWIN_CACHE_DIR = Var.with_default(
        pygwin_cache_dir,
        "This is the location where cache files used by pygwin are stored, such as commands-cache...",
        doc_default="``$XDG_CACHE_HOME/pygwin``",
        type_str="str",
    )
    ...
    PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE = Var.with_default(
        ...
        "(e.g. ``$PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE += ['/bin', '/sbin']``).",
        type_str="env_path",
    )

    PYGWIN_COMMANDS_CACHE_TRACE = Var.with_default(
        False,
        "If True, print trace messages showing where each command was resolved "
        "from (cached directory listing PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE vs. disk stat) "
...
class ChangeDirSetting(Xettings):
    ...
    CDPATH = Var.with_default(
        ...
        "with Bash, pygwin always prefer an existing relative path.",
        type_str="env_path",
    )
...
class InterpreterSetting(Xettings):
    ...
    PYGWIN_SUBPROC_ARG_EXPANDUSER = Var.with_default(
        True,
    ...
    FOREIGN_ALIASES_SUPPRESS_SKIP_MESSAGE = Var.with_default(
        ...
        "because it already exists in pygwin.",
        is_configurable=True,
    )
    FOREIGN_ALIASES_OVERRIDE = Var.with_default(
        False,
        "Whether or not foreign aliases should override pygwin aliases "
        "with the same name. Note that setting of this must happen in the "
        "environment that pygwin was started from. "
        "It cannot be set in the ``.pygwinrc`` as loading of foreign aliases happens before "
        "``.pygwinrc`` is parsed.",
        is_configurable=True,
    )
...
class PromptSetting(Xettings):
    ...
    PYGWIN_PROMPT_SHOW_SUBPROC_ERROR = Var.with_default(
        False,
        ...
        "shown — this flag only controls pygwin's own exception printout "
        "that follows it.\n\n"
        ...
        "* ``True``: pygwin additionally prints "
        ...
        "``pygwin -c``) are unaffected — there the exception propagates "
    ...
    PROMPT = Var(
        ...
        doc_default="``pygwin.environ.DEFAULT_PROMPT``",
    )
    ...
    PROMPT_TOKENS_FORMATTER = Var(
        ...
        doc_default="``pygwin.prompt.base.prompt_tokens_formatter_default``",
    )
    RIGHT_PROMPT = Var(
        is_string_or_callable,
        ensure_string,
    ...
    SUGGEST_COMMANDS = Var.with_default(
        True,
        "When a user types an invalid command, pygwin will try to offer "
        "suggestions of similar valid commands if this is True.",
    )
    SUGGEST_MAX_NUM = Var.with_default(
        5,
        "pygwin will show at most this many suggestions in response to an "
    ...
    TITLE = Var(
        ...
        ensure_string,
        DEFAULT_TITLE,
        "The title text for the window in which pygwin is running. Formatted "
        "in the same manner as ``$PROMPT``, see 'Customizing the Prompt' "
        "http://xon.sh/tutorial.html#customizing-the-prompt.",
        doc_default="``pygwin.environ.DEFAULT_TITLE``",
    )
    ...
    PYGWIN_HISTORY_MATCH_ANYWHERE = Var.with_default(
        False,
    ...
    PYGWIN_STDERR_PREFIX = Var.with_default(
        "",
        ...
        "conjunction with ``$PYGWIN_STDERR_POSTFIX`` to close out the block. "
        "For example, to have stderr appear on a red background, the "
        'prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}". '
        "It works with ``!()`` (colors will be reduced) or ``$PYGWIN_CAPTURE_ALWAYS=True``.",
    )
    PYGWIN_STDERR_POSTFIX = Var.with_default(
        "",
        "A format string, using the same keys and colors as ``$PROMPT``, that "
        "is appended whenever stderr is displayed. This may be used in "
        "conjunction with ``$PYGWIN_STDERR_PREFIX`` to start the block. "
        "For example, to have stderr appear on a red background, the "
        'prefix & postfix pair would be "{BACKGROUND_RED}" & "{RESET}". '
        "It works with ``!()`` (colors will be reduced) or ``$PYGWIN_CAPTURE_ALWAYS=True``.",
    )
    PYGWIN_SUPPRESS_WELCOME = Var.with_default(
        True,
...
class PromptHistorySetting(Xettings):
    ...
    PYGWIN_HISTORY_BACKEND = Var(
        ...
        "``PYGWIN_HISTORY_BACKEND`` also accepts a class type that inherits "
        "from ``pygwin.history.base.History``, or its instance.",
    )
    PYGWIN_HISTORY_FILE = Var.with_default(
        None,
    ...
    PYGWIN_STORE_STDOUT = Var.with_default(
        False,
        "Store the ``stdout`` and ``stderr`` streams to the history. "
        "Requires that PYGWIN_CAPTURE_ALWAYS is True.",
    )
    PYGWIN_HISTORY_SAVE_CWD = Var.with_default(
        True,
        "Save current working directory to the history.",
        doc_default="True",
    )
    PYGWIN_HISTORY_IGNORE_REGEX = Var(
        is_regex,
        to_itself,
        ensure_string,
        None,
        ...
        "Example: ``$PYGWIN_HISTORY_IGNORE_REGEX = '(^echo|^.*\\#\\#\\#\\s*|.*\\#\\#\\#\\s*$)'``"
        " - skip commands that start from ``echo`` or ``###``, or end from ``###``.",
    )
    PYGWIN_HISTORY_SIGINT_FLUSH = Var.with_default(
        True,
...
class PTKSetting(PromptSetting):  # sub-classing -> sub-group
    ...
    PYGWIN_PROMPT_AUTO_SUGGEST = Var.with_default(
        True,
        "Enable automatic command suggestions based on history."
        "\n\nPressing the right arrow key inserts the currently "
        "displayed suggestion. Set before starting the prompt e.g. in ``.pygwinrc`` file.",
        sync="AUTO_SUGGEST",
    )

    PYGWIN_PROMPT_NEXT_CMD = Var.with_default(
        "",
        "The text of the next command that will be inserted in the next prompt. "
        "Use ``<cursor>`` marker to set the cursor position, "
        "e.g. ``$PYGWIN_PROMPT_NEXT_CMD = 'echo <cursor> world'``.",
    )
    PYGWIN_PROMPT_NEXT_CMD_SUGGESTION = Var.with_default(
        "",
    ...
    PYGWIN_PROMPT_CURSOR_SHAPE = Var(
        always_false,
        to_ptk_cursor_shape,
        to_ptk_cursor_shape_display_value,
    ...
    PYGWIN_AUTOPAIR = Var.with_default(
        False,
        "Whether Pygwin will auto-insert matching parentheses, brackets, and "
        "quotes. Only available under the prompt-toolkit shell.",
    )
    PYGWIN_COPY_ON_DELETE = Var.with_default(
        False,
    ...
    PYGWIN_USE_SYSTEM_CLIPBOARD = Var.with_default(
        True,
    ...
    PYGWIN_CTRL_BKSP_DELETION = Var.with_default(
        False,
...
class AutoCompletionSetting(Xettings):
    ...
    ALIAS_COMPLETIONS_OPTIONS_BY_DEFAULT = Var.with_default(
        doc="If True, :py:class:`pygwin.completers.argparser.ArgparseCompleter` based completions "
    ...
    ALIAS_COMPLETIONS_OPTIONS_LONGEST = Var.with_default(
        ...
        "the :py:class:`pygwin.completers.argparser.ArgparseCompleter` based completions. "
    ...
    PYGWIN_COMPLETER_MODE = Var.with_default(
        "substring_tier",
    ...
    PYGWIN_BASH_PATH_OVERRIDE = Var.with_default(
        None,
        ...
        "``pygwin.platform_info.bash_command``.",
        doc_default="None",
    )
    PYGWIN_COMPLETER_DIRS = Var.with_default(
        default_completer_dirs,
        """\
A list of paths where Pygwin searches for command completions.
...
class WindowsSetting(Xettings):
    ...
    PYGWIN_WIN_CONSOLE_HISTORY_BUFFERS = Var.with_default(
        32,
        ...
        "exhausted by nested consoles (pygwin -> ``py`` -> ``python``), losing "
        "up-arrow history in the child REPL; ``0`` disables. For the other "
        "fields, set this to ``0`` and call "
        "``@.imp.pygwin.platforms.winutils.set_console_history_info("
        "nbuf=64, bufsize=1024, flags=1)`` from your pygwin RC.",
        is_configurable=ON_WINDOWS,
    )
...
class DeprecatedSetting(Xettings):  # sub-classing -> sub-group
    ...
    AUTO_SUGGEST = PTKSetting.PYGWIN_PROMPT_AUTO_SUGGEST.set_attrs(
        {
            "sync": "PYGWIN_PROMPT_AUTO_SUGGEST",
            "deprecated": True,
            "doc": "Deprecated. Use PYGWIN_PROMPT_AUTO_SUGGEST.",
        }
    )
    RAISE_SUBPROC_ERROR = SubprocessSetting.PYGWIN_SUBPROC_CMD_RAISE_ERROR.set_attrs(
        {
            "sync": "PYGWIN_SUBPROC_CMD_RAISE_ERROR",
            "deprecated": True,
            "doc": "Deprecated. Use PYGWIN_SUBPROC_CMD_RAISE_ERROR.",
        }
    )
...
class Env(cabc.MutableMapping):
    """A pygwin environment, whose variables have limited typing.
    ...
    * PYGWIN_HISTORY_SIZE: this variable is an (int | float, str) tuple.
    * LC_* (locale categories): locale category names get/set the Python
    ...
    def __init__(self, *args, **kwargs):
        ...
        # (``PYGWIN_SUBPROC_CMD_RAISE_ERROR``) are present in the source
        # dict — which happens whenever a parent pygwin process set the
        ...
        if "PATH" not in self._d:
            ...
            # it can be modified in-place in the pygwinrc file
        ...
        if self.get("UPDATE_OS_ENVIRON"):
            if self._orig_env is None:
                try:
                    ...
                    # Rollback to keep pygwin env and os.environ in sync.
...
def locate_binary(name):
    ...
    with scanning all files in ``$PATH``. First of all take a look into ``pygwin.specs.executables``
...
def pygwinrc_context(
    rcfiles=None, rcdirs=None, execer=None, ctx=None, env=None, login=True
):
    """
    Attempts to read in all pygwinrc files (and search pygwinrc directories),
    ...
    if rcfiles is not None:
        for rcfile in rcfiles:
            if os.path.isfile(rcfile):
                status = pygwin_script_run_control(
...
def windows_foreign_env_fixes(ctx):
    ...
    # which messes up these environment variables for pygwin.
...
def pygwin_script_run_control(filename, ctx, env, execer=None, login=True):
    """Loads a pygwin file and applies it as a run control.
    ...
    with swap_values(ctx, updates):
        ...
        if exc_info != (None, None, None):
            ...
            if err_type is SyntaxError:
                msg = "syntax error in pygwin run control file {0!r}: {1!s}"
            else:
                msg = "error running pygwin run control file {0!r}: {1!s}"
...
def default_env(env=None):
    ...
    # other shells' PROMPT definitions generally don't work in PYGWIN:
    try:
        del ctx["PROMPT"]
    except KeyError:
        pass
...
def save_origin_env_to_file(env, session_id):
    data_dir = env.get("PYGWIN_DATA_DIR", None)
    if data_dir is None:
        print(
            "pygwin: $PYGWIN_DATA_DIR is not set, skipping origin env save",
            file=sys.stderr,
        )
        return
    ...
    if os.access(env_file_name.parent, os.W_OK):
        ...
        env["PYGWIN_ORIGIN_ENV_FILE"] = str(env_file_name)
    else:
        print(f"pygwin: Write access denied for {data_dir!r}", file=sys.stderr)


def load_origin_env_from_file():
    e = os_environ
    if "PYGWIN_ORIGIN_ENV_FILE" not in e:
        print("pygwin: No env file to restore", file=sys.stderr)
        raise SystemExit(1)

    load_origin_env_file = Path(e["PYGWIN_ORIGIN_ENV_FILE"])
    if load_origin_env_file.is_file() and os.access(load_origin_env_file, os.R_OK):
        ...
        print(f"pygwin: Failed to load file {load_origin_env_file!r}", file=sys.stderr)
        raise SystemExit(1)
