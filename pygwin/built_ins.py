...
from pygwin.debug import PygwinDebug
from pygwin.lib.inspectors import Inspector
from pygwin.lib.lazyasd import lazyobject
from pygwin.platform_info import ON_POSIX, ON_WINDOWS
from pygwin.tools import (
    PygwinCalledProcessError,
    PygwinError,
    expand_path,
    globpath,
    on_main_thread,
...
def resetting_signal_handle(sig, f):
    ...
    # ValueError. When pygwin is set up from a non-main thread (e.g. an embedded
    # host calling pygwin.main.setup() off-main, like the 2020 virtualenv CI
    # case in pygwin/pygwin#3689), skip registration rather than crash — the host
    ...
    def new_signal_handler(s=None, frame=None):
        ...
        if sig == signal.SIGHUP:
            ...
            """
            import pygwin.procs.jobs as xj
...
class PygwinPathLiteralChangeDirectoryContextManager:
    """Implements context manager to use in pygwin path literal."""

    def __init__(self, path: PygwinPathLiteral):
        self.path = path

    def __enter__(self):
        self._pygwin_old_cwd = os.getcwd()
        os.chdir(self.path)
        return self.path

    def __exit__(self, exc_type, exc, tb):
        os.chdir(self._pygwin_old_cwd)
        return False


class PygwinPathLiteral(BasePath):  # type: ignore
    """Extension of ``pathlib.Path`` to support extended functionality."""

    def cd(self) -> PygwinPathLiteralChangeDirectoryContextManager:
        """Returns context manager to change the directory
        e.g. ``with p'/tmp'.cd(): $[ls]``
        """
        return PygwinPathLiteralChangeDirectoryContextManager(self)
...
class PygwinList(list):
    """List subclass returned by glob operations with convenience methods.

    All methods return PygwinList, enabling chaining::
    ...
    def unique(self):
        """Return a PygwinList with duplicates removed, preserving order."""
        return PygwinList(dict.fromkeys(self))

    def paths(self):
        """Convert string elements to pathlib.Path objects."""
        self._check_paths("paths")
        return PygwinList(pathlib.Path(p) for p in self)

    def sorted(self, key=None, reverse=False):
        """Return a new sorted PygwinList."""
        return PygwinList(builtins.sorted(self, key=key, reverse=reverse))

    def filter(self, func):
        """Return a PygwinList with only elements where func(elem) is truthy."""
        return PygwinList(x for x in self if func(x))

    def dirs(self):
        """Keep only paths that are existing directories."""
        self._check_paths("dirs")
        return PygwinList(p for p in self if os.path.isdir(p))

    def files(self):
        """Keep only paths that are existing files."""
        self._check_paths("files")
        return PygwinList(p for p in self if os.path.isfile(p))

    def exists(self):
        """Keep only paths that exist on disk."""
        self._check_paths("exists")
        return PygwinList(p for p in self if os.path.exists(p))
    ...
    def hidden(self):
        ...
        return PygwinList(p for p in self if self._is_hidden(p))
    ...
    def visible(self):
        ...
        return PygwinList(p for p in self if not self._is_hidden(p))


def path_literal(s):
    s = expand_path(s)
    return PygwinPathLiteral(s)


def regexsearch(s):
    s = expand_path(s)
    dotglob = XSH.env.get("DOTGLOB")
    return PygwinList(reglob(s, include_dotfiles=dotglob))
...
def pathsearch(func, s, pymode=False, pathobj=False):
    ...
    if not callable(func) or len(inspect.signature(func).parameters) != 1:
        ...
        raise PygwinError(error % func)
    o = func(s)
    if pathobj and pymode:
        o = PygwinList(map(pathlib.Path, o))
    no_match = PygwinList() if pymode else [s]
...
def _check_subproc_helper_raise(in_boolop):
    ...
    * ``$PYGWIN_SUBPROC_RAISE_ERROR`` is False.
    ...
    * ``@error_ignore`` was applied (``spec.raise_subproc_error is False``).
    ...
    if in_boolop:
        return
    if not XSH.env.get("PYGWIN_SUBPROC_RAISE_ERROR"):
        return
    cp = XSH.lastcmd
    if cp is None:
        return
...
def subproc_captured_stdout(*cmds, envs=None, in_boolop=False):
    ...
    that was produced as a str or list based on ``$PYGWIN_SUBPROC_OUTPUT_FORMAT``.

    ``in_boolop`` is set by the parser to True when this call is a direct
    operand of a ``&&``/``||`` chain, so the runtime can adjust behavior
    (e.g. suppress ``$PYGWIN_SUBPROC_CMD_RAISE_ERROR`` to let the chain
    short-circuit on returncode).
    """

    import pygwin.procs.specs

    r = pygwin.procs.specs.run_subproc(
...
def subproc_captured_inject(*cmds, envs=None, in_boolop=False):
    ...
    The string is split using pygwin's lexer, rather than Python's str.split()
    or shlex.split().
    """
    import pygwin.procs.specs

    o = pygwin.procs.specs.run_subproc(
        cmds, captured="stdout", envs=envs, in_boolop=in_boolop
    )
    _check_subproc_helper_raise(in_boolop)
...
def subproc_captured_object(*cmds, envs=None, in_boolop=False):
    ...
    return pygwin.procs.specs.run_subproc(
...
def subproc_check_boolop(value):
    ...
    * A standalone subproc-helper call at statement level — bare
    ...
    When ``$PYGWIN_SUBPROC_RAISE_ERROR`` is True (default), this enforces
...
def builtin_cmd(name):
    """Run a builtin name as a subprocess command if $PYGWIN_BUILTINS_TO_CMD is set
    and the name is a known command or alias. Otherwise return the builtin value."""
    env = XSH.env or {}
    if env.get("PYGWIN_BUILTINS_TO_CMD"):
...
def eval_fstring_field(field):
    """Evaluates the argument in Pygwin context."""
    res = XSH.execer.eval(
...
def enter_macro(obj, raw_block, glbs, locs):
    ...
    kind = getattr(obj, "__pygwin_block__", str)
...
@contextlib.contextmanager
def pygwin_builtins(execer=None):
    """A context manager for using the pygwin builtins only in a limited
...
class PygwinSessionInterface:
    """Pygwin Session Interface

    Attributes
    ----------
    env : pygwin.environ.Env
        A pygwin environment e.g. `@.env.get('HOME', '/tmp')`.

    history : pygwin.history.History
        Pygwin history backend e.g. `@.history[-1].cmd`.
        See also `history --help` to manage history from
        command line.

    imp : pygwin.built_ins.InlineImporter
        The inline importer provides instant access to library
        functions and attributes e.g. `@.imp.time.time()`.

    lastcmd : pygwin.procs.pipelines.CommandPipeline
        Last executed subprocess-mode command pipeline
        e.g. `@.lastcmd.rtn` returns exit code.

    debug : pygwin.debug.PygwinDebug
...
class PygwinSession:
    """All components defining a pygwin session.

    Warning! If you use this object for any reason and access ``__pygwin__``
    or ``pygwin.built_ins.XSH`` attributes or functions, you do so at your
    ...
    to improve ``PygwinSessionInterface`` or ``pygwin.api``.
    """
    ...
    def __init__(self):
        ...
        ----------
        exit: int or None
            Session attribute. In case of integer value it signals pygwin to exit
            with returning this value as exit code.
        """
        self.interface = PygwinSessionInterface()
        self.execer = None
        self.ctx = {}
        ...
        self.glob = lambda *a, **kw: PygwinList(globpath(*a, **kw))
    ...
    def completers(self):
        ...
        if self._completers is None:
            from pygwin.completers.init import default_completers
    ...
    def load(
        ...
        **kwargs,
        ...
        execer : Execer, optional
            Pygwin execution object, may be None to start
        ctx : Mapping, optional
            Context to start pygwin session with.
        inherit_env : bool
            If ``True``: inherit environment variables from ``os.environ``.
            If ``False``: use default values for environment variables and
            set ``$PYGWIN_ENV_INHERITED = False``.
        """
        from pygwin.commands_cache import CommandsCache
        from pygwin.environ import Env, default_env, save_origin_env_to_file

        if not hasattr(builtins, "__pygwin__"):
            builtins.__pygwin__ = self
        if ctx is not None:
            self.ctx = ctx
        ...
        if "env" in kwargs:
            ...
            no_env = {"PYGWIN_ENV_INHERITED": False}
    ...
    def link_builtins(self):
        ...
        for refname in self._initial_builtin_names:
            objname = f"__pygwin__.builtins.{refname}"
        ...
        # relies on __pygwin__.env in default aliases
...
def get_default_builtins(execer=None):
    ...
    return types.SimpleNamespace(
        PygwinError=PygwinError,
        PygwinCalledProcessError=PygwinCalledProcessError,
...
class DynamicAccessProxy:
    ...
    def _obj(self):
        ...
        results for ``getattr(builtins, "__pygwin__")`` (it doesn't advance
