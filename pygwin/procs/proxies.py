...
from pygwin.cli_utils import run_with_partial_args
from pygwin.lib.string import endswith_newline
...
class ProcProxyThread(threading.Thread):
    ...
    def __init__(
        ...
        if xt.on_main_thread():
            ...
            if xp.ON_WINDOWS:
                # Windows Ctrl+Break would otherwise terminate pygwin itself
    ...
    def _signal_break(self, signum, frame):
        ...
        callable alias instead of terminating pygwin itself (issue #4852).
        """
        if self._interrupted:
            return
        self._interrupted = True
