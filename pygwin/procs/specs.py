...
import pygwin.lib.lazyasd as xl
import pygwin.lib.lazyimps as xli
...
from pygwin.procs.pipelines import (
...
from pygwin.procs.proxies import ProcProxy, ProcProxyThread
from pygwin.procs.readers import ConsoleParallelReader
...
def get_script_subproc_command(fname, args):
    ...
    if xp.ON_WINDOWS:
        ...
        ext_upper = ext.upper()
        # 1) .xsh / .py / .pyw — run with the current pygwin interpreter
        #    (pygwin compiles .py as pure Python and .xsh as pygwin code
        #    via codecache, matching the Linux behaviour)
        if ext_upper in {".XSH", ".PY", ".PYW"}:
            return [sys.executable, "-m", "pygwin", fname] + args
...
def safe_open(fname, mode, buffering=-1):
    """Safely attempts to open a file in for pygwin subprocs."""
    # file descriptors
    try:
        kwargs = {"encoding": "utf-8"} if "b" not in mode else {}
        return open(fname, mode, buffering=buffering, **kwargs)
    except PermissionError as ex:
        raise xt.PygwinError(f"pygwin: {fname}: permission denied") from ex
    except FileNotFoundError as ex:
        raise xt.PygwinError(f"pygwin: {fname}: no such file or directory") from ex
    except Exception as ex:
        raise xt.PygwinError(f"pygwin: {fname}: unable to open file") from ex
    ...
    if dest.startswith("&"):
        try:
            ...
            if loc is None:
                ...
                raise xt.PygwinError(e)
        except (ValueError, xt.PygwinError):
            raise
        except Exception:
            pass
    mode = _MODES.get(mode, None)
    if mode == "r" and (len(orig) > 0 or len(dest) > 0):
        raise xt.PygwinError(f"Unrecognized redirection command: {r}")
...
class SubprocSpec:
    ...
    def __init__(
        ...
        self.output_format = XSH.env.get("PYGWIN_SUBPROC_OUTPUT_FORMAT", "stream_lines")
        self.raise_subproc_error = None  # Spec-based $PYGWIN_SUBPROC_CMD_RAISE_ERROR.
    ...
    def _run_binary(self, kwargs):
        if not self.cmd[0]:
            raise xt.PygwinError("pygwin: subprocess mode: command is empty")
        bufsize = 1
        try:
            ...
            e = "pygwin: subprocess mode: permission denied: {0}"
            raise xt.PygwinError(e.format(self.cmd[0])) from ex
        except FileNotFoundError as ex:
            cmd0 = self.cmd[0]
            if len(self.cmd) == 1 and cmd0.endswith("?"):
                ...
                if cmdq in XSH.aliases:
                    from pygwin.aliases import print_alias_help
            ...
            e = f"pygwin: subprocess mode: command not found: {repr(cmd0)}"
            env = XSH.env
            sug = xt.suggest_commands(cmd0, env)
            if len(sug.strip()) > 0:
                e += "\n" + sug
            if XSH.env.get("PYGWIN_INTERACTIVE"):
                ...
                for replacement in replacements:
                    ...
                    try:
                        ...
                        continue
            raise xt.PygwinError(e) from ex
        return p
    ...
    def prep_env_subproc(self, kwargs):
        ...
        with XSH.env.swap(self.env) as env:
            denv = env.detype()
        if xp.ON_WINDOWS:
            # Over write prompt variable as pygwin's $PROMPT does
    ...
    def prep_preexec_fn(self, kwargs, pipeline_group=None):
        ...
        kwargs["preexec_fn"] = pygwin_preexec_fn
    ...
    def resolve_executable_commands(self):
        ...
        try:
            ...
            if scriptcmd is not None:
                ...
                _, ext = os.path.splitext(fname)
                raise xt.PygwinError(
                    f"pygwin: {self.cmd[0]}: unknown file type {ext!r} — "
                    f"not in $PATHEXT."
                )
        except PermissionError as ex:
            e = "pygwin: subprocess mode: permission denied: {0}"
    ...
    def resolve_stack(self):
        ...
        if not stack[3][3].startswith("test_"):
            assert stack[3][3] == "run_subproc", "pygwin stack has changed!"
        del stack[:5]
        self.stack = stack
...
def _last_spec_update_threading(last: SubprocSpec):
    ...
    threadable = (
        ...
        and (captured != "hiddenobject" or env.get("PYGWIN_CAPTURE_ALWAYS"))
...
def _make_last_spec_captured(last: SubprocSpec):
    ...
    if last.stdout is not None:
        ...
        and not XSH.env.get("PYGWIN_CAPTURE_ALWAYS")
    ):
        last.universal_newlines = True
...
def _trace_specs(trace, specs, cmds, captured):
    ...
    ``trace`` is the value of ``$PYGWIN_SUBPROC_TRACE``. If it's a
    ...
    try:
        ...
        for i, redirect in enumerate(redirects):
            if redirect == "|":
                ...
                raise xt.PygwinError(f"unrecognized redirect {redirect!r}")
        ...
        for spec in specs:
            if spec._stdout is _PIPE_ALL or spec._stderr is _PIPE_ERR:
                raise xt.PygwinError(
                    "pygwin: redirect 'a>p'/'e>p' requires a following pipe '|'"
                )

        # Apply boundary conditions
        if not XSH.env.get("PYGWIN_CAPTURE_ALWAYS"):
...
def _set_specs_capture_always(specs_to_capture):
    """Set PYGWIN_CAPTURE_ALWAYS for all specs."""
    for spec in specs_to_capture:
        if spec.env is None:
            spec.env = {"PYGWIN_CAPTURE_ALWAYS": True}
        else:
            spec.env.setdefault("PYGWIN_CAPTURE_ALWAYS", True)


def _shell_set_title(cmds):
    if XSH.env.get("PYGWIN_INTERACTIVE") and XSH.shell is not None:
...
def run_subproc(cmds, captured=False, envs=None, in_boolop=False):
    ...
    if trace := XSH.env.get("PYGWIN_SUBPROC_TRACE", False):
