...
from pygwin.procs.jobs import proc_untraced_waitpid
...
class PopenThread(threading.Thread):
    ...
    def __init__(self, *args, stdin=None, stdout=None, stderr=None, **kwargs):
        ...
        self.store_stdin = env.get("PYGWIN_STORE_STDIN")
        self.timeout = env.get("PYGWIN_PROC_FREQUENCY")
        ...
        if xt.on_main_thread():
            ...
            if xp.ON_WINDOWS:
                ...
                # handler the default action would terminate pygwin itself
        ...
        if uninew:
            self.encoding = enc = env.get("PYGWIN_ENCODING")
            self.encoding_errors = err = env.get("PYGWIN_ENCODING_ERRORS")
    ...
    def _signal_break(self, signum, frame):
        ...
        stop its default action from terminating pygwin itself (issue #4852):
    ...
    def _enable_cbreak_stdin(self):
        ...
        try:
            ...
            # pygwin's tty device, such as in testing.
            self.stdin_mode = None
            return
        new = self.stdin_mode[:]
