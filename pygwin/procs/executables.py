...
from pygwin.lib.itertools import unique_everseen
...
# Directories listed in $PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE are scanned
...
def _get_stable_prefixes() -> tuple[str, ...]:
    """Return lowered realpath prefixes from $PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE.
    ...
    raw = env.get("PYGWIN_COMMANDS_CACHE_READ_DIR_ONCE", [])
    current = tuple(raw)
...
def _cache_debug(msg):
    """Print trace message if $PYGWIN_COMMANDS_CACHE_TRACE is True."""
    env = XSH.env if XSH.env is not None else {}
    if not env.get("PYGWIN_COMMANDS_CACHE_TRACE", False):
        return
...
def locate_file_in_path_env(name, env=None, check_executable=False, use_pathext=False):
    ...
    for path, possible_name in itertools.product(paths, possible_names):
        ...
        if cached is None:
            ...
            _cache_debug(
                f"pygwin-commands-cache: {prefix} `{result}` "
        ...
        try:
            ...
            result = str(filepath)
            _cache_debug(
                f"pygwin-commands-cache: get from disk `{result}` "
                f"({time.perf_counter() - t0:.4f} sec)"
            )
            return result
        except PermissionError:
    ...
    _cache_debug(
        f"pygwin-commands-cache: not found `{name}` ({time.perf_counter() - t0:.4f} sec)"
    )
