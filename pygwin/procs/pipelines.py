...
def update_process_group(pipeline_group, background):
    if not xp.ON_POSIX:
        return False
    env = XSH.env
    if not env.get("PYGWIN_INTERACTIVE"):
        return False
    if background:
        return True
...
class CommandPipeline:
    ...
    def __init__(self, specs):
        ...
        if xp.ON_POSIX and not xt.on_main_thread():
            ...
            # process group as pygwin. This fixes case 2 of issue #4277, where
        ...
        for i, spec in enumerate(specs):
            ...
            if proc.pid and pipeline_group is None and not spec.is_proxy:
                ...
                # pygwin, not child processes, so they cannot join the
                ...
                # Python object, so the terminal must stay with pygwin.
    ...
    def __repr__(self):
        debug = XSH.env.get("PYGWIN_DEBUG", False)
    ...
    def iterraw(self):
        ...
        if proc is None:
            return
        timeout = XSH.env.get("PYGWIN_PROC_FREQUENCY")
    ...
    def itercheck(self):
        ...
        if self.returncode:
            ...
            raise xt.PygwinCalledProcessError(
    ...
    def tee_stdout(self):
        ...
        from pygwin.procs.proxies import STDOUT_DISPATCHER
    ...
    def stream_stderr(self, lines):
        ...
        from pygwin.procs.proxies import STDERR_DISPATCHER
        ...
        show_stderr = self.captured != "object" or env.get(
            "PYGWIN_SUBPROC_CAPTURED_PRINT_STDERR", True
        )
        if show_stderr:
    ...
    def _signal_pipeline(self):
        ...
        * *pygwin's own group* — holds subprocesses spawned *inside*
          ...
          when Ctrl+C sends SIGINT to the foreground (pygwin) group;
        ...
        for p in self.procs:
            if p is not None and p.poll() is None:
                ...
                # Skip ProcProxyThread whose pid == pygwin's own pid
    ...
    def _raise_subproc_error(self):
        ...
        # the statement do it via $PYGWIN_SUBPROC_RAISE_ERROR.
        if XSH.env.get("PYGWIN_SUBPROC_CMD_RAISE_ERROR"):
            try:
                raise subprocess.CalledProcessError(rtn, spec.args, output=self.output)
            finally:
                # needed to get a working terminal in interactive mode
    ...
    def returncode(self):
        ...
        if rc is None:
            try:
                ...
                if XSH.env.get("PYGWIN_SUBPROC_TRACE", False):
                    xt.print_exception(
    ...
    def stderr_prefix(self):
        ...
        if p is None:
            env = XSH.env
            t = env.get("PYGWIN_STDERR_PREFIX")
    ...
    def stderr_postfix(self):
        ...
        if p is None:
            env = XSH.env
            t = env.get("PYGWIN_STDERR_POSTFIX")
            s = xt.format_std_prepost(t, env=env)
            p = s.encode(
                encoding=env.get("PYGWIN_ENCODING"),
                errors=env.get("PYGWIN_ENCODING_ERRORS"),
