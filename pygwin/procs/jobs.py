"""Job control for the pygwin shell."""
...
from pygwin.tools import get_signal_name, on_main_thread, print_warning, unthreadable
...
def proc_untraced_waitpid(proc, hang, task=None, raise_child_process_error=False):
    ...
    See also ``pygwin.tools.describe_waitpid_status()``.
...
def _kill_per_pid(job, signal):
    ...
    for pid in job["pids"]:
        ...
        try:
            ...
            print_warning(
                f"pygwin: permission denied sending {get_signal_name(signal)} to pid {pid}"
            )
...
if ON_DARWIN:
    ...
    def _send_signal(job, signal):
        try:
            ...
            print_warning(
                f"pygwin: permission denied sending {get_signal_name(signal)} to process group {job['pgrp']}"
            )
        except OSError:
    ...
    def _send_signal(job, signal):
        ...
        if pgrp is None:
            ...
            try:
                ...
                print_warning(
                    f"pygwin: permission denied sending {get_signal_name(signal)} to process group {pgrp}"
                )
...
def add_job(info):
    ...
    if (
        ...
        and info["bg"]
        and XSH.env.get("PYGWIN_INTERACTIVE")
    ):
        print_one_job(num)
        ...
        if get_jobs():
            ...
            if _last_exit_time and last_cmd_start and _last_exit_time > last_cmd_start:
                ...
                if XSH.env["SHELL_TYPE"] != "prompt_toolkit":
                    ...
                    # newline
                    print()
                print(f"pygwin: {msg}", file=sys.stderr)
...
def hup_all_jobs():
    """
    Send SIGHUP to all child processes (called when exiting pygwin).
    """
    _clear_dead_jobs()
