"""Lint rules for pygwin source code.

Each rule is a plain function ``(tree) -> list[Finding]`` operating on the
*transformed* pygwin AST (a standard :mod:`ast` ``Module`` in which subprocess
lines have been lowered to ``__pygwin__.subproc_*`` calls, so shell command
...
* ``XSH001`` — a ``$VAR`` whose name is unknown but is a close match for a real
  pygwin environment variable (likely a typo).
* ``XSH002`` — a string literal assigned to a ``$VAR`` whose registered
  validator rejects it (e.g. ``$PYGWIN_STORE_STDOUT = 'yes'`` instead of
  ``True``).
* ``XSH003`` — use of a deprecated environment variable.
* ``XSH101`` — an import bound name that is never used (pyflakes' F401).

The env-var rules draw their source of truth from :data:`pygwin.environ.DEFAULT_VARS`
...
def _known_env_vars():
    # Imported lazily: keeps ``import pygwin.linter`` cheap and avoids paying the
    # environ import cost unless a file is actually linted.
    from pygwin.environ import DEFAULT_VARS

    return DEFAULT_VARS


def _env_var_name(node):
    """If *node* is ``__pygwin__.env['NAME']`` (a ``$VAR`` / ``${'NAME'}`` in
...
def check_env_vars(tree) -> list[Finding]:
    ...
    # classic mistake (``$PYGWIN_STORE_STDOUT = 'yes'``), while bare ``0``/``1``
...
#: All rules run by :func:`pygwin.linter.cli.lint_source`, in order.
