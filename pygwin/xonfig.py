"""The pygwin configuration (xonfig) utility."""
...
from pygwin.prompt.base import is_template_string
...
HR = "'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'`-.,_,.-*'"
WIZARD_HEAD = f"""
          {{BOLD_WHITE}}Welcome to the pygwin configuration wizard!{{RESET}}
          {{YELLOW}}------------------------------------------{{RESET}}
This will present a guided tour through setting up the pygwin static
config file. Pygwin will automatically ask you if you want to run this
...
For the configuration to take effect, you will need to restart pygwin.
...
{HR}
                      ...
                      {{YELLOW}}-------------------{{RESET}}
The pygwin shell has the ability to interface with Bash or zsh
via the foreign shell interface.

For configuration, this means that pygwin can load the environment,
...
makes it easier to transition to and from pygwin.
"""
...
{HR}
                  ...
                  {{YELLOW}}--------------------------{{RESET}}
The pygwin shell also allows you to setup environment variables from
the static configuration file. Any variables set in this way are
superseded by the definitions in the pygwinrc or on the command line.
...
def _dump_xonfig_foreign_shell(path, value):
    ...
    if cmd[0] == "source-foreign":
        cmd.append(shell)
    cmd.append('"echo loading pygwin foreign shell"')
    return " ".join(cmd)
...
def make_xonfig_wizard(default_file=None, confirm=False, no_wizard_file=None):
    """Makes a configuration wizard for pygwin config file.
    ...
    if confirm:
        q = (
            "Would you like to run the pygwin configuration wizard now?\n\n"
...
def _wizard(
    ...
    -------
    rcfile : -f, --file
        config file location, default=$PYGWINRC
    confirm : -c, --confirm
        confirm that the wizard should be run.
    """
    env = XSH.env
    shell = XSH.shell.shell
    pygwinrcs = env.get("PYGWINRC", [])
    fname = pygwinrcs[-1] if pygwinrcs and rcfile is None else rcfile
    no_wiz = os.path.join(env.get("PYGWIN_CONFIG_DIR"), "no-wizard")
    w = make_xonfig_wizard(default_file=fname, confirm=confirm, no_wizard_file=no_wiz)
    tempenv = {"PROMPT": "", "PYGWIN_STORE_STDOUT": False}
    pv = wiz.PromptVisitor(w, store_in_history=False, multiline=False)

    @contextlib.contextmanager
    def force_hide():
        if env.get("PYGWIN_STORE_STDOUT") and hasattr(shell, "_force_hide"):
...
def _info(
    ...
    """
    env = XSH.env
    data: list[tp.Any] = [("pygwin", PYGWIN_VERSION)]
    hash_, date_ = githash()
    if hash_:
    ...
    data.extend(
        [
            ...
            ("history backend", env.get("PYGWIN_HISTORY_BACKEND")),
    ...
    data.extend(
        [
            ...
            ("pygwin encoding", env.get("PYGWIN_ENCODING")),
            ("encoding errors", env.get("PYGWIN_ENCODING_ERRORS")),
        ]
    )
    ...
    envs = {
        ...
        "PYGWIN_CAPTURE_ALWAYS": None,
        "PYGWIN_SUBPROC_OUTPUT_FORMAT": None,
        "THREAD_SUBPROCS": None,
        "ENABLE_ASYNC_PROMPT": True,
        "ENABLE_COMMANDS_CACHE": False,
        "PYGWIN_CACHE_EVERYTHING": True,
        "PYGWIN_CACHE_SCRIPTS": None,
    }
...
def _styles(to_json=False, _stdout=None):
    """Prints available pygwin color styles
    ...
    """
    env = XSH.env
    curr = env.get("PYGWIN_COLOR_STYLE")
...
def _tok_colors(cmap, cols):
    from pygwin.style_tools import Color
...
def _colors(
    ...
    style_stash = XSH.env["PYGWIN_COLOR_STYLE"]

    if style is not None:
        if style not in color_style_names():
            print(f"Invalid style: {style}")
            return
        XSH.env["PYGWIN_COLOR_STYLE"] = style
    ...
    print_color(s)
    XSH.env["PYGWIN_COLOR_STYLE"] = style_stash
...
def _web(
    ...
    from pygwin.webconfig import main

    main.serve(browser)


class XonfigAlias(ArgParserAlias):
    """Manage pygwin configuration."""
...
WELCOME_MSG = [
    "",
    ("Welcome to the pygwin shell {version}", "^", " "),
    "",
    ...
    "",
    (
        "{{INTENSE_BLACK}}Create ~/.pygwinrc file manually or use xonfig to suppress the welcome message",
        "^",
        " ",
    ),
    "",
    "{{INTENSE_BLACK}}Start from commands:",
    "  {{GREEN}}xonfig{{RESET}} web         {{INTENSE_BLACK}}# Run the configuration tool in the browser to create ~/.pygwinrc {{RESET}}",
    "  {{GREEN}}xonfig{{RESET}} tutorial    {{INTENSE_BLACK}}# Open the pygwin tutorial in the browser{{RESET}}",
    "[SHELL_TYPE_WARNING]",
    "",
    ("{{INTENSE_BLACK}}", "<", "-"),
    "",
...
def print_welcome_screen():
    ...
    subst = dict(tagline=random.choice(list(TAGLINES)), version=PYGWIN_VERSION)
        ...
        if elem == "[SHELL_TYPE_WARNING]":
            if shell_type != "prompt_toolkit":
                print_color(
                    ...
                    f"  {{GREEN}}xpip{{RESET}} install -U 'pygwin[full]'"
                )
            continue
