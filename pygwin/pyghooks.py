...
from pygwin.color_tools import (
    BASE_PYGWIN_COLORS,
    RE_BACKGROUND,
    RE_PYGWIN_COLOR,
    find_closest_color,
...
from pygwin.lib.lazyimps import html, os_listxattr, terminal256
from pygwin.parsers.tokenize import SubprocCommentHighlight
...
from pygwin.platforms.win_colors import hardcode_colors_for_win10
from pygwin.procs.executables import locate_executable
from pygwin.pygments_cache import add_custom_style, get_style_by_name
from pygwin.style_tools import DEFAULT_STYLE_DICT, norm_name
...
def color_name_to_pygments_code(name, styles):
    """Converts a pygwin color name to a pygments color code."""
    token = getattr(Color, norm_name(name))
    if token in styles:
        return styles[token]
    m = RE_PYGWIN_COLOR.match(name)
    if m is None:
...
def color_token_by_name(xc: tuple, styles=None) -> _TokenType:
    """Returns (color) token corresponding to Pygwin color tuple, side effect: defines token is defined in styles"""
    if not styles:
        try:
...
class PygwinStyle(Style):
    """A pygwin pygments style that will dispatch to the correct color map
    ...
    def style_name(self, value):
        ...
        if value not in STYLES:
            try:  # loading style dynamically
                ...
                value = "default"
                XSH.env["PYGWIN_COLOR_STYLE"] = value
        cmap = STYLES[value]
        if value == "default":
            self._smap = PYGWIN_BASE_STYLE.copy()
        else:
            try:
        ...
        for file_type, pygwin_color in XSH.env.get("LS_COLORS", {}).items():
            color_token = color_token_by_name(pygwin_color, self.styles)
...
def pygwin_style_proxy(styler):
    """Factory for a proxy class to a pygwin style."""
...
def register_custom_pygments_style(
    ...
    # Overlay PYGWIN_BASE_STYLE so that the custom style inherits
    # pygwin's ANSI color names instead of pygments' hex codes.
    if base == "default":
        for token, value in PYGWIN_BASE_STYLE.items():
...
def make_pygments_style(palette):
    ...
    for name, t in BASE_PYGWIN_COLORS.items():
...
def _monkey_patch_pygments_codes():
    ...
    if not getattr(pygments.console, "_pygwin_patched", False):
        patched_codes = {}
        ...
        pygments.console._pygwin_patched = True
...
@lazyobject
def PygwinTerminal256Formatter():
    if (
        ptk_version_info()
    ...
    class PygwinTerminal256FormatterProxy(terminal256.Terminal256Formatter):
        """Proxy class for pygwin terminal256 formatting that understands.
        pygwin color tokens.
        """
    ...
    return PygwinTerminal256FormatterProxy


@lazyobject
def PygwinHtmlFormatter():
    ...
    class PygwinHtmlFormatterProxy(html.HtmlFormatter):
        """Proxy class for pygwin HTML formatting that understands.
        pygwin color tokens.
        """
    ...
    return PygwinHtmlFormatterProxy
...
def _is_plugin_mode():
    """True when PygwinLexer should fall back to optimistic tokens
    ...
    * **No live session** — the lexer is loaded as a pygments entry
      ...
      call ``pygwin.main.setup()``.  ``XSH.commands_cache`` is None,
      ``XSH.env`` is the mock ``{}`` set by ``PygwinLexer.__init__``,
      ``XSH.ctx`` is None — every runtime check fails by default and
      the rendered output ends up flooded with red Error markers.

    * **Explicit opt-in via** ``PygwinLexer.docs_mode`` — covers the
      case where pygwin *is* fully bootstrapped (so ``commands_cache``
      is set) but the lexer is still being used for offline rendering
      rather than a live prompt: typically a Sphinx ``conf.py`` that
      imports pygwin modules during configuration, after which
    ...
    """
    return PygwinLexer.docs_mode or getattr(XSH, "commands_cache", None) is None
...
def _command_is_valid(cmd):
    ...
    # pygwin session), since the singleton has no commands_cache yet.
...
class PygwinLexer(Python3Lexer):
    ...
    tokens = {
        ...
        "root": [
            ...
            # pygwin path-string literals (p prefix)
...
class PygwinConsoleLexer(PygwinLexer):
    """Pygwin console lexer for pygments.

    A pygwincon block is a sequence of lines with the following conventions:

    * ``@ ...`` — an interactive pygwin command. Tokenised with the ``@ ``
    ...
    * A line starting with two leading spaces — a continuation of the
      ...
      the normal pygwin highlighting.
    ...
    older pygwincon blocks that have bare output lines at column 0 — any
    ...
    tokens = {
        "root": [
            ...
            # rest of the line flows into normal pygwin highlighting.
            (r"(\n)(  )", bygroups(Text, Generic.Prompt)),
