"""Lazy imports that may apply across the pygwin package."""

import importlib
import os

from pygwin.lib.lazyasd import LazyObject, lazyobject
from pygwin.platform_info import ON_DARWIN, ON_WINDOWS

pygments = LazyObject(
    lambda: importlib.import_module("pygments"), globals(), "pygments"
)
pyghooks = LazyObject(
    lambda: importlib.import_module("pygwin.pyghooks"), globals(), "pyghooks"
)
...
def winutils():
    if ON_WINDOWS:
        import pygwin.platforms.winutils as m
    else:
        m = None
    return m
...
def macutils():
    if ON_DARWIN:
        import pygwin.platforms.macutils as m
    else:
        m = None
    return m
