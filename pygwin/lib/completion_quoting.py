...
shell-quoting in pygwin.

Used by both the built-in path completer
(:mod:`pygwin.completers.path`) and the bash-completion bridge
(:mod:`pygwin.completers.bash_completion`), which historically carried
...
:mod:`pygwin.completers.bash_completion` requires.
"""
...
# Characters the pygwin parser would treat specially in a subprocess
...
# since those are pygwin keywords.
_PATTERN = re.compile(
    r"""[\s`$\{\}\[\]\,\*\(\)"'\?&#|;<>"""
...
def name_needs_quotes(name: str, sep: str | None = None) -> bool:
    """Return True if *name* would be mangled by the pygwin parser and
