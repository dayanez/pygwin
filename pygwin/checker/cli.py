"""Command-line interface for ``pygwin check`` and the ``-n`` flag.
...
:func:`pygwin.main.premain` and delegates to :func:`check_no_execute` here, so
both spellings share one engine (:func:`check_source`).

Behaviour
---------
* ``pygwin check FILE...`` checks each path and prints ``FILE: OK`` per file
  (suppressed with ``-q``); ``-`` reads from stdin.
* ``pygwin -n`` checks the ``-c`` command, the script file, or piped stdin —
...
def _get_execer():
    """Return a process-wide :class:`~pygwin.execer.Execer`, with a session.
    ...
    ``pygwin check`` run. ``Execer.compile`` is a synchronous, fully-resetting
    ...
    (:mod:`pygwin.parsers.ast`) and the subprocess-recovery helpers
...
def check_source(src: str, filename: str = "<check>", mode: str = "exec", execer=None):
    """Parse and compile pygwin *src* to validate its syntax, without running it.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog=_PROG,
        description="Check pygwin source files for syntax errors without running them.",
    )
    p.add_argument(
        "files",
        nargs="+",
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for ``pygwin check FILE...``."""
    ...
    # exit-code ordering of the sibling ``pygwin format`` subcommand.
    if had_read_error:
        return EXIT_ERROR
    if had_syntax_error:
        return EXIT_SYNTAX
...
def check_no_execute(args) -> int:
    """Implements ``pygwin -n`` / ``--no-execute``.
    ...
    as :func:`pygwin.main.premain`, and stays silent on success.
    """
    execer = _get_execer()

    if args.command is not None:
        # ``-c`` runs in "single" mode at runtime; match it so the check
        # agrees exactly with what ``pygwin -c`` would compile.
    ...
    if args.file is not None:
        ...
        if os.path.isdir(path):
            print(f"pygwin: {args.file}: Is a directory.", file=sys.stderr)
            return EXIT_ERROR
        try:
            with open(path, encoding="utf-8") as fh:
                src = fh.read()
        except OSError as exc:
            print(f"pygwin: {args.file}: {exc.strerror or exc}", file=sys.stderr)
            return EXIT_ERROR
    ...
    print(
        "pygwin: -n/--no-execute: nothing to check "
