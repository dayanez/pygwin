...
from pygwin.platform_info import ON_POSIX, ON_WINDOWS, pathbasename
...
class CommandsCache(cabc.Mapping):
    ...
    Note! There is ``pygwin.procs.executables`` module with resolving executables.
    ...
    def __init__(self, env, aliases=None) -> None:
        ...
        self.env = env
        if aliases is None:
            from pygwin.aliases import Aliases, make_default_aliases
    ...
    def cache_file(self):
        ...
        if self._cache_file is None:
            if "PYGWIN_CACHE_DIR" in env and env.get(
                "COMMANDS_CACHE_SAVE_INTERMEDIATE"
            ):
                self._cache_file = (
                    Path(env["PYGWIN_CACHE_DIR"]).joinpath(self.CACHE_FILE).resolve()
                )
            else:
    ...
    def locate_binary(self, name, ignore_alias=False):
        ...
        NOT RECOMMENDED. Take a look into `pygwin.procs.executables.locate_executable`
...
def default_threadable_predictors():
    ...
    predictors = {
        ...
        "xo": predict_help_ver,
        "pygwin.sh": predict_shell,
        "pygwin": predict_shell,
