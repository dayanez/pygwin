"""The base class for pygwin shell"""
...
from pygwin.prompt.base import PromptFormatter, multiline_prompt
...
class _TeeStdBuf(io.RawIOBase):
    ...
    def __init__(
        ...
        encoding : str or None, optional
            ...
            rather than a binary one. Defaults to $PYGWIN_ENCODING if None.
        errors : str or None, optional
            ...
            $PYGWIN_ENCODING_ERRORS if None.
        ...
        self.membuf = membuf
        env = XSH.env
        self.encoding = env.get("PYGWIN_ENCODING") if encoding is None else encoding
        self.errors = env.get("PYGWIN_ENCODING_ERRORS") if errors is None else errors
        self.prestd = prestd
...
class Tee:
    ...
    def __init__(
        ...
        env = XSH.env
        prestderr = format_std_prepost(env.get("PYGWIN_STDERR_PREFIX"))
        poststderr = format_std_prepost(env.get("PYGWIN_STDERR_POSTFIX"))
        self.stderr = _TeeStd(
...
class BaseShell:
    ...
    def styler(self):
        if self._styler is DefaultNotGiven:
            if HAS_PYGMENTS:
                from pygwin.pyghooks import PygwinStyle

                env = XSH.env
                self._styler = PygwinStyle(env.get("PYGWIN_COLOR_STYLE"))
            else:
                self._styler = None
        return self._styler
    ...
    def precmd(self, line):
        ...
        try:
            ...
            # pygwin's own record of where the session is, so it stays closer
    ...
    def default(self, line, raw_line=None):
        ...
        ts1 = None
        enc = env.get("PYGWIN_ENCODING")
        err = env.get("PYGWIN_ENCODING_ERRORS")
        ...
        try:
            ...
            # print pygwin's ``CalledProcessError: ...`` line is governed
            # by ``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR``.
            #
            ...
            #
            # Scripts (``./script.xsh`` / ``pygwin -c``) never reach this
            ...
            if is_error_raise or env.get("PYGWIN_PROMPT_SHOW_SUBPROC_ERROR"):
            ...
            if (
                tee_out
                and env.get("PYGWIN_APPEND_NEWLINE")
    ...
    def _fix_cwd(self):
        ...
        if cwd is None:
            ...
            if pwd is None:
                ...
                msg += "pygwin: working directory does not exist: " + pwd
                msg += "{RESET}"
    ...
    def settitle(self):
        ...
        if ON_WINDOWS and "ANSICON" not in env:
            ...
            with open(1, "wb", closefd=False) as f:
                # prevent pygwin from answering interactive questions
    ...
    def print_color(self, string, hide=False, **kwargs):
        ...
        if isinstance(string, str):
            ...
            style_proxy = pyghooks.pygwin_style_proxy(self.styler)
            formatter = pyghooks.PygwinTerminal256Formatter(style=style_proxy)
