"""The prompt_toolkit based pygwin shell."""
...
# When pygwin runs inside an SSH session, suppress prompt-toolkit's Cursor
...
from pygwin.lib.lazyimps import pyghooks, pygments, winutils
from pygwin.platform_info import HAS_PYGMENTS, ON_POSIX, ON_WINDOWS, win_ansi_support
...
from pygwin.shells.ptk_shell.completer import PromptToolkitCompleter
from pygwin.shells.ptk_shell.formatter import PTKPromptFormatter
from pygwin.shells.ptk_shell.history import PromptToolkitHistory, _cust_history_matches
from pygwin.shells.ptk_shell.input_hook import PTKInputHook
from pygwin.shells.ptk_shell.key_bindings import load_pygwin_bindings
from pygwin.style_tools import DEFAULT_STYLE_DICT, _TokenType, partial_color_tokenize
from pygwin.tools import carriage_return, print_exception, print_warning
...
Fired after prompt toolkit has been initialized. Use this event in pygwin RC file.
...
def disable_copy_on_deletion():
    ...
    def handle_binding(name):
        ...
        if getattr(binding, "pygwin_disabled_clipboard", False):
            # binding's clipboard has already been disabled
            return

        binding.pygwin_disabled_clipboard = True
...
class PromptToolkitShell(BaseShell):
    """The pygwin shell for prompt_toolkit v2 and later."""
    ...
    def __init__(self, **kwargs):
        if not XSH.env.get("PYGWIN_DEBUG", False):
        ...
        if ON_WINDOWS:
            ...
            if nbuf := XSH.env.get("PYGWIN_WIN_CONSOLE_HISTORY_BUFFERS"):
                try:
        ...
        if not XSH.env.get("PYGWIN_COPY_ON_DELETE", False):
            disable_copy_on_deletion()
        if HAVE_SYS_CLIPBOARD and (XSH.env.get("PYGWIN_USE_SYSTEM_CLIPBOARD", True)):
        ...
        self.key_bindings = load_pygwin_bindings(ptk_bindings)
        ...
        def handler_before_render(app):
            if not app.current_buffer.text and (
                suggestion := XSH.env.get("PYGWIN_PROMPT_NEXT_CMD_SUGGESTION")
            ):
                app.current_buffer.suggestion = Suggestion(suggestion)
                XSH.env["PYGWIN_PROMPT_NEXT_CMD_SUGGESTION"] = ""
    ...
    def get_lazy_ptk_kwargs(self):
        ...
        if HAS_PYGMENTS:
            ...
            yield "lexer", PygmentsLexer(pyghooks.PygwinLexer)
    ...
    def get_prompt_style(self):
        ...
        if (
            ...
            print_warning(
                "$PTK_STYLE_OVERRIDES is deprecated, use $PYGWIN_STYLE_OVERRIDES instead!"
            )
            self._overrides_deprecation_warning_shown = True
        style_overrides_env.update(env.get("PYGWIN_STYLE_OVERRIDES", {}))

        if HAS_PYGMENTS:
            style = _style_from_pygments_cls(pyghooks.pygwin_style_proxy(self.styler))
    ...
    def singleline(
        ...
        if next_cmd_raw := env.get("PYGWIN_PROMPT_NEXT_CMD", ""):
            env["PYGWIN_PROMPT_NEXT_CMD"] = ""
        ...
        auto_suggest = auto_suggest if env.get("PYGWIN_PROMPT_AUTO_SUGGEST") else None
        ...
        if HAS_PYGMENTS:
            self.styler.style_name = env.get("PYGWIN_COLOR_STYLE")
        ...
        if env.get("PYGWIN_HISTORY_MATCH_ANYWHERE"):
        ...
        cursor_shape = env.get("PYGWIN_PROMPT_CURSOR_SHAPE")
        if cursor_shape:
        ...
        # immediately.
        self.prompter.app.ttimeoutlen = float(env.get("PYGWIN_PTK_TTIMEOUTLEN", 0.05))
        self.prompter.app.timeoutlen = float(env.get("PYGWIN_PTK_TIMEOUTLEN", 1.0))
    ...
    def continuation_tokens(self, width, line_number, is_soft_wrap=False):
        ...
        # Resolve both pygwin colors ({RED}) and ANSI escapes (\033[31m)
    ...
    def format_color(self, string, hide=False, force_string=False, **kwargs):
        ...
        if force_string and HAS_PYGMENTS:
            env = XSH.env
            style_overrides_env = env.get("PYGWIN_STYLE_OVERRIDES", {})
            self.styler.style_name = env.get("PYGWIN_COLOR_STYLE")
            self.styler.override(style_overrides_env)
            proxy_style = pyghooks.pygwin_style_proxy(self.styler)
            formatter = pyghooks.PygwinTerminal256Formatter(style=proxy_style)
    ...
    def print_color(self, string, end="\n", **kwargs):
        ...
        if HAS_PYGMENTS:
            ...
            proxy_style = _style_from_pygments_cls(
                pyghooks.pygwin_style_proxy(self.styler)
            )
        else:
    ...
    def color_style_names(self):
        ...
        if not HAS_PYGMENTS:
            return ["For other pygwin styles, please install pygments"]
