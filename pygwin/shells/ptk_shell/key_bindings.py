"""Key bindings for prompt_toolkit pygwin shell."""
...
from pygwin.aliases import pygwin_exit
...
def carriage_return(b, cli, *, autoindent=True):
    ...
    pygwin parser for execution or should insert a newline for continued input.
...
def can_compile(src):
    """Returns whether the code can be compiled, i.e. it is valid pygwin."""
...
def autopair_condition():
    """Check if PYGWIN_AUTOPAIR is set"""
    return XSH.env.get("PYGWIN_AUTOPAIR", False)
...
def load_pygwin_bindings(ptk_bindings: KeyBindingsBase) -> KeyBindingsBase:
    """
    ...
    if XSH.env["PYGWIN_CTRL_BKSP_DELETION"]:
