"""The readline based pygwin shell.
...
import pygwin.completers.tools as xct
...
from pygwin.lib.lazyasd import LazyObject, lazyobject
...
from pygwin.prompt.base import multiline_prompt
...
def setup_readline():
    ...
    if os.path.isfile(inputrc_name):
        try:
            ...
            print_exception("pygwin: could not load readline default init file.")
...
def _rebind_case_sensitive_completions():
    ...
    # Without this, readline may reject candidates before pygwin's
    # own completer sees them.  Pygwin handles case preference via
...
def rl_variable_value(variable):
    ...
    env = XSH.env
    enc, errors = env.get("PYGWIN_ENCODING"), env.get("PYGWIN_ENCODING_ERRORS")
...
class ReadlineShell(BaseShell, cmd.Cmd):
    ...
    def prompt(self):
        ...
        p = ansi_partial_color_format(p, style=env.get("PYGWIN_COLOR_STYLE"), hide=hide)
