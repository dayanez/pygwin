...
from pygwin.platform_info import ON_CYGWIN, ON_MSYS, ON_WINDOWS
from pygwin.tools import ensure_string, to_bool

COMMAND = """{seterrprevcmd}
{prevcmd}
echo __PYGWIN_ENV_BEG__
{envcmd}
echo __PYGWIN_ENV_END__
echo __PYGWIN_ALIAS_BEG__
{aliascmd}
echo __PYGWIN_ALIAS_END__
echo __PYGWIN_FUNCS_BEG__
{funcscmd}
echo __PYGWIN_FUNCS_END__
{postcmd}
{seterrpostcmd}"""
...
_FOREIGN_OUTPUT_FIRST_MARKER = "__PYGWIN_ENV_BEG__"


def _emit_foreign_script_output(stdout_str, stderr_str):
    """Write the sourced script's own stdout/stderr to pygwin's terminal.

    The captured stdout from the foreign shell contains both the script's
    own writes and the pygwin marker output (``__PYGWIN_ENV_BEG__`` and
...
def foreign_shell_data(
    ...
    """Extracts data from a foreign (non-pygwin) shells. Currently this gets
    ...
    show_output : bool, optional
        Whether to forward the sourced script's own stdout/stderr to pygwin's
        terminal. When False (the default) the script's writes are silently
        discarded — only the pygwin-injected env/alias/func dumps are parsed
    ...
    if shkey is None:
        ...
        sys.stderr.write(
            f"pygwin: error: unknown foreign shell {shell!r}. Supported: {supported}\n"
        )
        sys.stderr.flush()
        return None, None
    ...
    try:
        ...
        sys.stderr.write(f"pygwin: error: foreign shell not found: {shell!r} ({e})\n")
        sys.stderr.flush()
        if not safe:
            raise
        return None, None
...
@lazyobject
def ENV_RE():
    return re.compile("__PYGWIN_ENV_BEG__\n(.*)__PYGWIN_ENV_END__", flags=re.DOTALL)
...
@lazyobject
def ALIAS_RE():
    return re.compile("__PYGWIN_ALIAS_BEG__\n(.*)__PYGWIN_ALIAS_END__", flags=re.DOTALL)
...
def FUNCS_RE():
    return re.compile(
        "__PYGWIN_FUNCS_BEG__\n(.+)\n__PYGWIN_FUNCS_END__", flags=re.DOTALL
    )
...
class ForeignShellBaseAlias:
    ...
    def __call__(
        ...
        if streaming:
            ...
            # foreign shell inherits pygwin's terminal fds and writes around
            # the pipe pygwin set up for capture (issue #5043).
    ...
    def _is_streaming(args):
        """Test and modify args if ``--pygwin-nostream`` is present."""
        if "--pygwin-nostream" not in args:
            return args, True
        warnings.warn(
            "--pygwin-nostream is deprecated and will be removed in a future "
...
def load_foreign_aliases(shells):
    ...
    for shell in shells:
        ...
        if not XSH.env.get("FOREIGN_ALIASES_OVERRIDE"):
            for alias in set(shaliases) & set(pygwin_aliases):
                del shaliases[alias]
                if XSH.env.get("PYGWIN_DEBUG", 0) >= 1:
                    print(
                        f"aliases: ignoring alias {alias!r} of shell {shell['shell']!r} "
                        "which tries to override pygwin alias.",
                        file=sys.stderr,
                    )
