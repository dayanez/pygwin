"""Tests for ``$PYGWIN_SUBPROC_RAISE_ERROR`` and the AST wrapping that
implements its semantics (see ``pygwin.parsers.base.wrap_subproc_raise_checks``
and ``pygwin.built_ins.subproc_check_boolop``).

The spec these tests pin down:

* ``$PYGWIN_SUBPROC_RAISE_ERROR`` (default ``True``) raises a
  ``CalledProcessError`` when the *final* command of a statement
  (standalone or the result of an ``&&``/``||`` chain) returns a
  non-zero exit status.
* ``$PYGWIN_SUBPROC_CMD_RAISE_ERROR`` (default ``False``) is independent
...
from pygwin.parsers.base import wrap_subproc_raise_checks
...
def _unparse(tree):
    ...
    return ast.unparse(tree).replace("__pygwin__.", "_xs.")


def test_wrap_standalone_bare_command(pygwin_execer):
    """Bare ``ls nono`` becomes
    ``subproc_check_boolop(subproc_captured_hiddenobject(...))``.
    """
    tree = pygwin_execer.parse("ls nono\n", ctx=None)
    src = _unparse(tree)
    assert "_xs.subproc_check_boolop(_xs.subproc_captured_hiddenobject" in src


def test_wrap_boolop_chain(pygwin_execer):
    """``echo 1 && echo 2`` wraps the BoolOp and tags both operands
    with ``in_boolop=True``.
    """
    tree = pygwin_execer.parse("echo 1 && echo 2\n", ctx=None)
    src = _unparse(tree)
...
def test_wrap_does_not_double_wrap_boolop_in_stmt(pygwin_execer):
...
def test_wrap_skips_captured_object_form(pygwin_execer):
    """``cp = !(ls nono)`` MUST NOT be wrapped — captured forms are the
    user's responsibility.
    """
    tree = pygwin_execer.parse("cp = !(ls nono)\n", ctx=None)
    src = _unparse(tree)
    assert "subproc_check_boolop" not in src
    assert "subproc_captured_object" in src


def test_wrap_includes_captured_stdout_form(pygwin_execer):
...
def test_wrap_includes_uncaptured_bracket_form(pygwin_execer):
...
def test_wrap_skips_pure_python_boolop(pygwin_execer):
    """Pure-Python ``a and b`` (no subproc helpers) is left untouched."""
    tree = pygwin_execer.parse("x = 1 and 2\n", ctx=None)
    src = _unparse(tree)
    assert "subproc_check_boolop" not in src


def test_wrap_semicolon_separated_statements(pygwin_execer):
    """``echo 1 || echo 2; ls nono`` wraps both statements: the BoolOp
    on the first, and the standalone command on the second.
    """
    tree = pygwin_execer.parse("echo 1 || echo 2; ls nono\n", ctx=None)
    src = _unparse(tree)
    # Two distinct wrappers — one per statement.
    assert src.count("subproc_check_boolop") == 2


def test_wrap_subproc_raise_checks_is_idempotent(pygwin_execer):
...
@pytest.fixture
def raise_env(pygwin_session, monkeypatch):
    """Default-ish env for the new semantics."""
    monkeypatch.setitem(pygwin_session.env, "PYGWIN_SUBPROC_CMD_RAISE_ERROR", False)
    monkeypatch.setitem(pygwin_session.env, "PYGWIN_SUBPROC_RAISE_ERROR", True)
    return pygwin_session.env


@skip_if_on_windows
def test_standalone_failure_raises(pygwin_execer, raise_env):
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope_path_no_exist__\n")


@skip_if_on_windows
def test_standalone_success_does_not_raise(pygwin_execer, raise_env):
    pygwin_execer.exec("echo ok\n")


@skip_if_on_windows
def test_and_chain_both_succeed(pygwin_execer, raise_env):
    pygwin_execer.exec("echo a && echo b\n")


@skip_if_on_windows
def test_and_chain_first_fails_raises(pygwin_execer, raise_env):
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope__ && echo never\n")


@skip_if_on_windows
def test_and_chain_second_fails_raises(pygwin_execer, raise_env):
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo ok && ls /__nope__\n")


@skip_if_on_windows
def test_or_chain_first_succeeds(pygwin_execer, raise_env):
    """``echo ok || ls /__nope__`` — first succeeds, second never runs."""
    pygwin_execer.exec("echo ok || ls /__nope__\n")


@skip_if_on_windows
def test_or_chain_first_fails_second_succeeds(pygwin_execer, raise_env):
    pygwin_execer.exec("ls /__nope__ || echo fb\n")


@skip_if_on_windows
def test_or_chain_both_fail_raises(pygwin_execer, raise_env):
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope1__ || ls /__nope2__\n")


@skip_if_on_windows
def test_semicolon_second_stmt_fails_raises(pygwin_execer, raise_env):
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo a || echo b; ls /__nope__\n")


@skip_if_on_windows
def test_semicolon_both_succeed(pygwin_execer, raise_env):
    pygwin_execer.exec("echo a; echo b\n")


@skip_if_on_windows
def test_captured_object_form_does_not_raise(pygwin_execer, raise_env):
    """``!()`` is full capture; user takes responsibility."""
    pygwin_execer.exec("p = !(ls /__nope__)\n")


@skip_if_on_windows
def test_captured_stdout_form_raises(pygwin_execer, raise_env):
    """Per spec only ``!(...)`` is the responsibility-opt-out form;
    ``$(...)`` still raises on a failing command.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("p = $(ls /__nope__ 2>/dev/null)\n")


@skip_if_on_windows
def test_uncaptured_bracket_form_raises(pygwin_execer, raise_env):
    """``$[ls nono]`` raises even though the helper returns ``None``;
    the wrapper falls back to ``XSH.lastcmd`` to read the rc.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("$[ls /__nope__]\n")


@skip_if_on_windows
def test_inject_form_raises(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_inject_inside_outer_command_raises(pygwin_execer, raise_env):
    ...
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo @$(ls /__nope__)\n")


@skip_if_on_windows
def test_disabling_raise_error_var(pygwin_execer, raise_env, monkeypatch):
    """``PYGWIN_SUBPROC_RAISE_ERROR=False`` reverts to non-raising."""
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_RAISE_ERROR", False)
    pygwin_execer.exec("ls /__nope__\n")
    pygwin_execer.exec("ls /__nope__ && echo never\n")


@skip_if_on_windows
def test_cmd_raise_error_overrides_chain_skip(pygwin_execer, raise_env, monkeypatch):
    """``PYGWIN_SUBPROC_CMD_RAISE_ERROR=True`` raises on the *first*
    failing pipeline in a chain — even before the BoolOp short-circuit
    has a chance to fall through.
    """
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_CMD_RAISE_ERROR", True)
    with pytest.raises(CalledProcessError):
...
@skip_if_on_windows
def test_error_ignore_in_chain(pygwin_execer, raise_env):
    """``echo 1 && @error_ignore ls /__nope__`` — final pipeline is the
    failing ``ls``, but ``@error_ignore`` suppresses the raise.
    """
    pygwin_execer.exec("echo 1 && @error_ignore ls /__nope__\n")


@skip_if_on_windows
def test_error_ignore_standalone(pygwin_execer, raise_env):
    pygwin_execer.exec("@error_ignore ls /__nope__\n")


@skip_if_on_windows
def test_error_raise_standalone(pygwin_execer, raise_env, monkeypatch):
    """``@error_raise`` raises even when CMD_RAISE_ERROR is False."""
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_CMD_RAISE_ERROR", False)
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("@error_raise ls /__nope__\n")


@skip_if_on_windows
def test_error_raise_in_chain_raises_immediately(pygwin_execer, raise_env):
    """``@error_raise`` raises mid-chain even when ``||`` would otherwise
    rescue, because the user explicitly opted in to raise on this cmd.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("@error_raise ls /__nope__ || echo fb\n")
...
@skip_if_on_windows
def test_pipe_failure_raises(pygwin_execer, raise_env):
    """A pipeline whose final stage fails raises just like a single
    command — ``echo hi | grep x`` ends with rc=1 from grep.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo hi | grep x\n")


@skip_if_on_windows
def test_three_cmd_and_chain_last_fails(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_three_cmd_and_chain_first_fails(pygwin_execer, raise_env):
    """``cmd1 && cmd2 && cmd3`` where the first fails — short-circuits
    on the first failing pipeline; later commands never run.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope__ && echo never1 && echo never2\n")


@skip_if_on_windows
def test_mixed_and_or_chain_rescued(pygwin_execer, raise_env):
    ...
    """
    pygwin_execer.exec("ls /__nope__ && echo n || echo fb\n")


@skip_if_on_windows
def test_mixed_and_or_chain_middle_fail_rescued(pygwin_execer, raise_env):
    """``echo a && ls nono || echo fb`` — ``ls`` (the second op of the
    inner ``and``) fails; the outer ``or`` falls back to ``echo fb``.
    """
    pygwin_execer.exec("echo a && ls /__nope__ || echo fb\n")


@skip_if_on_windows
def test_error_ignore_overrides_cmd_raise_error(pygwin_execer, raise_env, monkeypatch):
    """``@error_ignore`` wins over ``$PYGWIN_SUBPROC_CMD_RAISE_ERROR=True``."""
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_CMD_RAISE_ERROR", True)
    pygwin_execer.exec("@error_ignore ls /__nope__\n")
...
def test_error_raise_overrides_disabled_raise_error(
    pygwin_execer, raise_env, monkeypatch
):
    """``@error_raise`` raises even when *both* ``$PYGWIN_SUBPROC_RAISE_ERROR``
    and ``$PYGWIN_SUBPROC_CMD_RAISE_ERROR`` are False — the decorator is the
...
@skip_if_on_windows
def test_called_process_error_attributes(pygwin_execer, raise_env):
    """The raised ``CalledProcessError`` carries the right ``returncode``
    and ``cmd`` so user code can ``except`` and inspect it.
    """
    with pytest.raises(CalledProcessError) as exc_info:
        pygwin_execer.exec("ls /__nope_specific__\n")
    err = exc_info.value
    assert err.returncode != 0
    # ``cmd`` is the original argv list pygwin passed to the subprocess.
    assert "ls" in err.cmd
    assert "/__nope_specific__" in err.cmd


@skip_if_on_windows
def test_statement_after_raise_does_not_run(pygwin_execer, raise_env, capsys):
...
@skip_if_on_windows
def test_three_cmd_or_chain_all_fail(pygwin_execer, raise_env):
    """``fail || fail || fail`` — all fail, chain result is falsy → raise."""
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__n1__ || ls /__n2__ || ls /__n3__\n")


@skip_if_on_windows
def test_three_cmd_or_chain_last_succeeds(pygwin_execer, raise_env):
    """``fail || fail || echo ok`` — last succeeds, chain result is truthy."""
    pygwin_execer.exec("ls /__n1__ || ls /__n2__ || echo ok\n")


@skip_if_on_windows
def test_three_cmd_or_chain_middle_succeeds(pygwin_execer, raise_env):
    """``fail || echo ok || fail`` — middle succeeds, ``or`` short-circuits."""
    pygwin_execer.exec("ls /__n1__ || echo ok || ls /__n3__\n")


@skip_if_on_windows
def test_three_cmd_or_chain_first_succeeds(pygwin_execer, raise_env):
    """``echo ok || fail || fail`` — first succeeds, rest never evaluated."""
    pygwin_execer.exec("echo ok || ls /__n2__ || ls /__n3__\n")


@skip_if_on_windows
def test_or_then_and_chain_rescued(pygwin_execer, raise_env):
    ...
    """
    pygwin_execer.exec("ls /__nope__ || echo ok && echo yes\n")


@skip_if_on_windows
def test_or_then_and_chain_second_part_fails(pygwin_execer, raise_env):
    """``fail || echo ok && fail`` — ``fail or (ok and fail)`` →
    inner ``and`` returns ``fail`` → that's the chain result → raise.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope__ || echo ok && ls /__n2__\n")


@skip_if_on_windows
def test_and_then_or_all_fail(pygwin_execer, raise_env):
    """``fail && echo n || fail`` — ``(fail and echo_n) or fail2``.
    Inner ``and`` short-circuits to ``fail`` (falsy), outer ``or``
    evaluates ``fail2`` (also falsy) → raise.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__n1__ && echo n || ls /__n2__\n")


@skip_if_on_windows
def test_pipe_in_and_chain_pipe_fails(pygwin_execer, raise_env):
    """``echo ok | grep x && echo n`` — pipe exits non-zero (grep no match),
    ``and`` short-circuits → chain result is the failing pipe → raise.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo ok | grep x && echo n\n")


@skip_if_on_windows
def test_pipe_in_or_chain_pipe_fails_rescued(pygwin_execer, raise_env):
    """``echo ok | grep x || echo fb`` — pipe fails, ``or`` evaluates
    ``echo fb`` (success) → no raise.
    """
    pygwin_execer.exec("echo ok | grep x || echo fb\n")


@skip_if_on_windows
def test_pipe_in_and_chain_pipe_succeeds(pygwin_execer, raise_env):
    """``echo ok | grep ok && echo yes`` — pipe succeeds, ``and`` continues,
    ``echo yes`` succeeds → no raise.
    """
    pygwin_execer.exec("echo ok | grep ok && echo yes\n")


@skip_if_on_windows
def test_error_ignore_mid_and_chain(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_error_raise_first_in_or_chain(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_error_raise_second_in_and_chain(pygwin_execer, raise_env):
    """``echo ok && @error_raise fail`` — first succeeds, ``and`` continues,
    ``@error_raise`` on the second raises immediately.
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo ok && @error_raise ls /__nope__\n")


@skip_if_on_windows
def test_error_raise_on_success_does_not_raise(pygwin_execer, raise_env):
    """``@error_raise echo ok`` — command succeeds, no raise despite decorator."""
    pygwin_execer.exec("@error_raise echo ok\n")


@skip_if_on_windows
def test_both_env_vars_true_standalone_fail(pygwin_execer, raise_env, monkeypatch):
    """Both ``$PYGWIN_SUBPROC_RAISE_ERROR`` and ``$PYGWIN_SUBPROC_CMD_RAISE_ERROR``
...
@skip_if_on_windows
def test_both_env_vars_true_chain_first_fail(pygwin_execer, raise_env, monkeypatch):
...
@skip_if_on_windows
def test_both_env_vars_false_no_raise(pygwin_execer, raise_env, monkeypatch):
    """Both env vars False — nothing raises."""
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_RAISE_ERROR", False)
    monkeypatch.setitem(raise_env, "PYGWIN_SUBPROC_CMD_RAISE_ERROR", False)
    pygwin_execer.exec("ls /__nope__\n")
    pygwin_execer.exec("ls /__n1__ && echo n\n")
    pygwin_execer.exec("ls /__n1__ || ls /__n2__\n")


@skip_if_on_windows
def test_error_ignore_overrides_both_env_vars(pygwin_execer, raise_env, monkeypatch):
...
@skip_if_on_windows
def test_captured_object_in_chain_does_not_raise(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_semicolon_first_fails_second_succeeds(pygwin_execer, raise_env):
    """``fail; echo ok`` — first statement raises before the second runs."""
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__nope__; echo ok\n")


@skip_if_on_windows
def test_semicolon_first_succeeds_second_fails(pygwin_execer, raise_env):
...
@skip_if_on_windows
def test_four_cmd_and_chain_all_succeed(pygwin_execer, raise_env):
    """``a && b && c && d`` — all succeed → no raise."""
    pygwin_execer.exec("echo a && echo b && echo c && echo d\n")


@skip_if_on_windows
def test_four_cmd_or_chain_all_fail(pygwin_execer, raise_env):
    """``f || f || f || f`` — all fail → raise."""
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("ls /__n1__ || ls /__n2__ || ls /__n3__ || ls /__n4__\n")


@skip_if_on_windows
def test_four_cmd_or_chain_last_succeeds(pygwin_execer, raise_env):
    """``f || f || f || echo ok`` — last succeeds → no raise."""
    pygwin_execer.exec("ls /__n1__ || ls /__n2__ || ls /__n3__ || echo ok\n")


@skip_if_on_windows
def test_pipe_success_standalone(pygwin_execer, raise_env):
    """``echo hello | grep hello`` — pipe succeeds → no raise."""
    pygwin_execer.exec("echo hello | grep hello\n")


@skip_if_on_windows
def test_pipe_failure_standalone(pygwin_execer, raise_env):
    """``echo hello | grep nope`` — pipe fails → raise."""
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("echo hello | grep nope\n")


@skip_if_on_windows
def test_error_raise_with_cmd_raise_error_false(pygwin_execer, raise_env, monkeypatch):
...
@skip_if_on_windows
def test_error_ignore_in_or_chain_all_fail(pygwin_execer, raise_env):
    ...
    """
    with pytest.raises(CalledProcessError):
        pygwin_execer.exec("@error_ignore ls /__n1__ || ls /__n2__\n")


@skip_if_on_windows
def test_error_ignore_last_in_or_chain(pygwin_execer, raise_env):
    ...
    """
    pygwin_execer.exec("ls /__n1__ || @error_ignore ls /__n2__\n")


# ---------------------------------------------------------------------------
# Interactive prompt display — ``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR``
#
...
# unaffected by ``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR``.
# ---------------------------------------------------------------------------


@pytest.fixture
def base_shell(pygwin_execer, pygwin_session):
    ...
    shell = BaseShell(pygwin_execer, pygwin_session.ctx, completer=False)
    return shell


@skip_if_on_windows
def test_prompt_suppresses_calledprocesserror_by_default(base_shell, raise_env, capsys):
    """With the default ``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR=False`` the
...
def test_prompt_shows_calledprocesserror_when_opted_in(
    ...
    """``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR=True`` restores the historical
    behavior of printing the ``CalledProcessError`` after a failing
    command at the prompt.
    """
    monkeypatch.setitem(raise_env, "PYGWIN_PROMPT_SHOW_SUBPROC_ERROR", True)
    base_shell.default("ls /__nope_specific__\n")
    err = capsys.readouterr().err
...
def test_prompt_always_shows_error_raise_decorator(base_shell, raise_env, capsys):
    ...
    ``$PYGWIN_PROMPT_SHOW_SUBPROC_ERROR``.
    """
...
def test_prompt_last_return_code_set_even_when_suppressed(
    base_shell, raise_env, pygwin_session
):
