...
from pygwin.pytest.tools import ON_WINDOWS, skip_if_on_unix, skip_if_on_windows


@pytest.fixture
def check_eval(pygwin_execer, pygwin_session, monkeypatch):
    def factory(input):
        env = {
            "AUTO_CD": False,
        ...
        for key, val in env.items():
            monkeypatch.setitem(pygwin_session.env, key, val)
        if ON_WINDOWS:
            monkeypatch.setitem(
                pygwin_session.env, "PATHEXT", [".COM", ".EXE", ".BAT", ".CMD"]
            )
        pygwin_execer.eval(input)
        return True
...
def test_ls_dashl(pygwin_execer_parse):
    assert pygwin_execer_parse("ls -l")


def test_which_ls(pygwin_execer_parse):
    assert pygwin_execer_parse("which ls")


def test_echo_hello(pygwin_execer_parse):
    assert pygwin_execer_parse("echo hello")


def test_echo_star_with_semi(pygwin_execer_parse):
    assert pygwin_execer_parse("echo * spam ; ![echo eggs]\n")


def test_simple_func(pygwin_execer_parse):
    code = "def prompt():\n    return '{user}'.format(user='me')\n"
    assert pygwin_execer_parse(code)


def test_lookup_alias(pygwin_execer_parse):
    code = 'def foo(a,  s=None):\n    return "bar"\n@(foo)\n'
    assert pygwin_execer_parse(code)


def test_lookup_anon_alias(pygwin_execer_parse):
    code = 'echo "hi" | @(lambda a, s=None: a[0]) foo bar baz\n'
    assert pygwin_execer_parse(code)


def test_simple_func_broken(pygwin_execer_parse):
    code = "def prompt():\n    return '{user}'.format(\n       user='me')\n"
    assert pygwin_execer_parse(code)


def test_bad_indent(pygwin_execer_parse):
    code = "if True:\nx = 1\n"
    with pytest.raises(SyntaxError):
        pygwin_execer_parse(code)


def test_keyword_arg_with_non_name_lhs_raises_syntax_error(pygwin_execer_parse):
...
def test_non_default_after_default_arg_raises_syntax_error(pygwin_execer_parse):
...
def test_comment_colon_ending(pygwin_execer_parse):
...
def test_bad_rhs_subproc(pygwin_execer_parse):
...
def test_indent_with_empty_line(pygwin_execer_parse):
    code = "if True:\n\n    some_command for_sub_process_mode\n"
    assert pygwin_execer_parse(code)


def test_command_in_func(pygwin_execer_parse):
    code = "def f():\n    echo hello\n"
    assert pygwin_execer_parse(code)


def test_command_in_func_with_comment(pygwin_execer_parse):
    code = "def f():\n    echo hello # comment\n"
    assert pygwin_execer_parse(code)


def test_pyeval_redirect(pygwin_execer_parse):
    code = 'echo @("foo") > bar\n'
    assert pygwin_execer_parse(code)


def test_pyeval_multiline_str(pygwin_execer_parse):
    code = 'echo @("""hello\nmom""")\n'
    assert pygwin_execer_parse(code)


def test_echo_comma(pygwin_execer_parse):
    code = "echo ,\n"
    assert pygwin_execer_parse(code)


def test_echo_comma_val(pygwin_execer_parse):
    code = "echo ,1\n"
    assert pygwin_execer_parse(code)


def test_echo_comma_2val(pygwin_execer_parse):
    code = "echo 1,2\n"
    assert pygwin_execer_parse(code)


def test_echo_line_cont(pygwin_execer_parse):
    code = 'echo "1 \\\n2"\n'
    assert pygwin_execer_parse(code)
...
def test_two_echo_line_cont(code, pygwin_execer_parse):
    assert pygwin_execer_parse(code)
...
def test_annotated_assign(pygwin_execer_exec):
    # issue #3959 - didn't work because of `CtxAwareTransformer`
    assert pygwin_execer_exec("x : int = 42")


def test_exec_eol(pygwin_execer_exec):
    locs = dict()
    assert pygwin_execer_exec("a=0", locs=locs) and pygwin_execer_exec(
        "a=0\n", locs=locs
    )


def test_exec_print(capsys, pygwin_execer_exec):
    ls = {"nested": "some long list"}
    pygwin_execer_exec("print(ls)", locs=dict(ls=ls))
    out, err = capsys.readouterr()
    assert out.strip() == repr(ls)


def test_exec_function_scope(pygwin_execer_exec):
    # issue 4363
    assert pygwin_execer_exec("x = 0; (lambda: x)()")
    assert pygwin_execer_exec("x = 0; [x for _ in [0]]")


def test_exec_scope_reuse(pygwin_execer_exec):
    # Scopes should not be reused between execs.
    # A first-pass incorrect solution to issue 4363 made this mistake.
    assert pygwin_execer_exec("x = 0")
    with pytest.raises(NameError):
        pygwin_execer_exec("print(x)")
...
# the call and produced ``![[subscript]]``.  With ``$PYGWIN_SUBPROC_RAISE_ERROR``
...
def test_call_subscript_preserved_at_stmt_level(expr, pygwin_execer_parse):
    """Forward-referenced ``call()[subscript]`` must not collapse to a subproc."""
    tree = pygwin_execer_parse(expr + "\n")
...
def test_forward_ref_call_subscript_in_function(pygwin_execer_parse):
...
def test_fstring_statement_stays_python(expr, pygwin_execer_parse):
    tree = pygwin_execer_parse(expr + "\n")
...
def test_pyeval_in_andor_chain(line, pygwin_execer_parse):
    """``cmd && @(...) && cmd`` must parse without a SyntaxError."""
    assert pygwin_execer_parse(line + "\n")
